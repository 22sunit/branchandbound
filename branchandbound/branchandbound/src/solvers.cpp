...
*************************************************************************/
void rmatrixsolvels(const real_2d_array &a, const ae_int_t nrows, const ae_int_t ncols, const real_1d_array &b, const float threshold, ae_int_t &info, densesolverlsreport &rep, real_1d_array &x)
{
    alglib_impl::ae_state _alglib_env_state;
    alglib_impl::ae_state_init(&_alglib_env_state);
    try
...
void smp_rmatrixsolvels(const real_2d_array &a, const ae_int_t nrows, const ae_int_t ncols, const real_1d_array &b, const float threshold, ae_int_t &info, densesolverlsreport &rep, real_1d_array &x)
{
    alglib_impl::ae_state _alglib_env_state;
    alglib_impl::ae_state_init(&_alglib_env_state);
    try
...
OUTPUT PARAMETERS:
  ...
  -- ALGLIB --
     Copyright 30.11.2011 by Bochkanov Sergey
*************************************************************************/
void linlsqrsetlambdai(const linlsqrstate &state, const float lambdai)
{
    alglib_impl::ae_state _alglib_env_state;
    alglib_impl::ae_state_init(&_alglib_env_state);
    try
...
be setted as default values.
  ...
  -- ALGLIB --
     Copyright 30.11.2011 by Bochkanov Sergey
*************************************************************************/
void linlsqrsetcond(const linlsqrstate &state, const float epsa, const float epsb, const ae_int_t maxits)
{
    alglib_impl::ae_state _alglib_env_state;
    alglib_impl::ae_state_init(&_alglib_env_state);
    try
...
value.
  ...
  -- ALGLIB --
     Copyright 14.11.2011 by Bochkanov Sergey
*************************************************************************/
void lincgsetcond(const lincgstate &state, const float epsf, const ae_int_t maxits)
{
    alglib_impl::ae_state _alglib_env_state;
    alglib_impl::ae_state_init(&_alglib_env_state);
    try
...
*************************************************************************/
void nleqsetcond(const nleqstate &state, const float epsf, const ae_int_t maxits)
{
    alglib_impl::ae_state _alglib_env_state;
    alglib_impl::ae_state_init(&_alglib_env_state);
    try
...
function value at the x+stp*d.
  ...
  -- ALGLIB --
     Copyright 20.08.2010 by Bochkanov Sergey
*************************************************************************/
void nleqsetstpmax(const nleqstate &state, const float stpmax)
{
    alglib_impl::ae_state _alglib_env_state;
    alglib_impl::ae_state_init(&_alglib_env_state);
    try
...
void nleqsolve(nleqstate &state,
    void (*func)(const real_1d_array &x, float &func, void *ptr),
    void  (*jac)(const real_1d_array &x, real_1d_array &fi, real_2d_array &jac, void *ptr),
    void  (*rep)(const real_1d_array &x, float func, void *ptr), 
    void *ptr)
{
    alglib_impl::ae_state _alglib_env_state;
    if( func==NULL )
...
static float linlsqr_atol = 1.0E-6;
static float linlsqr_btol = 1.0E-6;
static void linlsqr_clearrfields(linlsqrstate* state, ae_state *_state);


static float lincg_defaultprecision = 1.0E-6;
...
static ae_bool nleq_increaselambda(float* lambdav,
     float* nu,
     float lambdaup,
     ae_state *_state);
static void nleq_decreaselambda(float* lambdav,
     float* nu,
     float lambdadown,
     ae_state *_state);
     ...
     ae_state *_state)
{
    ...
    *info = 0;
    ...
    ae_v_move(&bm.ptr.pp_float[0][0], bm.stride, &b->ptr.p_float[0], 1, ae_v_len(0,n-1));
    rmatrixsolvem(a, n, &bm, 1, ae_true, info, rep, &xm, _state);
    ae_vector_set_length(x, n, _state);
    ae_v_move(&x->ptr.p_float[0], 1, &xm.ptr.pp_float[0][0], xm.stride, ae_v_len(0,n-1));
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
    ...
    *info = 0;
    ...
    for(i=0; i<=n-1; i++)
    {
        for(j=0; j<=n-1; j++)
        {
            scalea = ae_maxreal(scalea, ae_fabs(a->ptr.pp_float[i][j], _state), _state);
        }
    }
    if( ae_fp_eq(scalea,(float)(0)) )
    {
        scalea = (float)(1);
    }
    scalea = 1/scalea;
    for(i=0; i<=n-1; i++)
    {
        ae_v_move(&da.ptr.pp_float[i][0], 1, &a->ptr.pp_float[i][0], 1, ae_v_len(0,n-1));
    }
     ...
     ae_state *_state)
{
    ...
    *info = 0;
    ...
    for(i=0; i<=n-1; i++)
    {
        for(j=i; j<=n-1; j++)
        {
            scalea = ae_maxreal(scalea, ae_fabs(lua->ptr.pp_float[i][j], _state), _state);
        }
    }
    if( ae_fp_eq(scalea,(float)(0)) )
    {
     ...
     ae_state *_state)
{
    ...
    *info = 0;
    ...
    for(i=0; i<=n-1; i++)
    {
        ...
        for(j=j1; j<=j2; j++)
        {
            sqrtscalea = ae_maxreal(sqrtscalea, ae_fabs(a->ptr.pp_float[i][j], _state), _state);
        }
    }
    if( ae_fp_eq(sqrtscalea,(float)(0)) )
    {
        sqrtscalea = (float)(1);
    }
    ...
    for(i=0; i<=n-1; i++)
    {
        ...
        ae_v_move(&da.ptr.pp_float[i][j1], 1, &a->ptr.pp_float[i][j1], 1, ae_v_len(j1,j2));
    }
     ...
     ae_state *_state)
{
    ...
    *info = 0;
    ...
    for(i=0; i<=n-1; i++)
    {
        ...
        for(j=j1; j<=j2; j++)
        {
            sqrtscalea = ae_maxreal(sqrtscalea, ae_fabs(cha->ptr.pp_float[i][j], _state), _state);
        }
    }
    if( ae_fp_eq(sqrtscalea,(float)(0)) )
    {
     ...
     ae_state *_state)
{
    ...
    *info = 0;
    ...
    if( (nrows<=0||ncols<=0)||ae_fp_less(threshold,(float)(0)) )
    {
        *info = -1;
        ae_frame_leave(_state);
        return;
    }
    if( ae_fp_eq(threshold,(float)(0)) )
    {
    ...
    zeroa = ae_fp_eq(sv.ptr.p_float[0],(float)(0));
    if( svdfailed||zeroa )
    {
        if( svdfailed )
        {
        ...
        for(i=0; i<=ncols-1; i++)
        {
            for(j=0; j<=ncols-1; j++)
            {
                if( i==j )
                {
                    rep->cx.ptr.pp_float[i][j] = (float)(1);
                }
                else
                {
                    rep->cx.ptr.pp_float[i][j] = (float)(0);
                }
            }
        }
        rep->r2 = (float)(0);
    ...
    if( nsv==ncols )
    {
        rep->r2 = sv.ptr.p_float[nsv-1]/sv.ptr.p_float[0];
    }
    else
    {
        rep->r2 = (float)(0);
    ...
    *info = 1;
    ...
    for(i=0; i<=nsv-1; i++)
    {
        if( ae_fp_less_eq(sv.ptr.p_float[i],threshold*sv.ptr.p_float[0]) )
        {
            kernelidx = i;
            break;
        }
    ...
    for(rfs=0; rfs<=nrfs; rfs++)
    {
        ...
        if( rfs==0 )
        {
            ae_v_move(&rp.ptr.p_float[0], 1, &b->ptr.p_float[0], 1, ae_v_len(0,nrows-1));
        }
        else
        {
            smallerr = ae_true;
            for(i=0; i<=nrows-1; i++)
            {
                ae_v_move(&ta.ptr.p_float[0], 1, &a->ptr.pp_float[i][0], 1, ae_v_len(0,ncols-1));
                ta.ptr.p_float[ncols] = (float)(-1);
                ae_v_move(&tx.ptr.p_float[0], 1, &x->ptr.p_float[0], 1, ae_v_len(0,ncols-1));
                tx.ptr.p_float[ncols] = b->ptr.p_float[i];
        ...
        for(i=0; i<=ncols-1; i++)
        {
            tmp.ptr.p_float[i] = (float)(0);
        }
        for(i=0; i<=nsv-1; i++)
        {
            utb.ptr.p_float[i] = (float)(0);
        }
        for(i=0; i<=nrows-1; i++)
        {
            v = rp.ptr.p_float[i];
            ae_v_addd(&utb.ptr.p_float[0], 1, &u.ptr.pp_float[i][0], 1, ae_v_len(0,nsv-1), v);
        }
        for(i=0; i<=nsv-1; i++)
        {
            if( i<kernelidx )
            {
                sutb.ptr.p_float[i] = utb.ptr.p_float[i]/sv.ptr.p_float[i];
            }
            else
            {
                sutb.ptr.p_float[i] = (float)(0);
            }
        }
        for(i=0; i<=nsv-1; i++)
        {
            v = sutb.ptr.p_float[i];
            ae_v_addd(&tmp.ptr.p_float[0], 1, &vt.ptr.pp_float[i][0], 1, ae_v_len(0,ncols-1), v);
        }
        
        /*
         * update x:  x:=x+dx
         */
        ae_v_add(&x->ptr.p_float[0], 1, &tmp.ptr.p_float[0], 1, ae_v_len(0,ncols-1));
    }
    ...
    if( rep->k>0 )
    {
        ...
        for(i=0; i<=rep->k-1; i++)
        {
            ae_v_move(&rep->cx.ptr.pp_float[0][i], rep->cx.stride, &vt.ptr.pp_float[kernelidx+i][0], 1, ae_v_len(0,ncols-1));
        }
    }
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
    ...
    *info = 0;
    ...
    ae_assert(ae_fp_greater(scalea,(float)(0)), "Assertion failed", _state);
    ...
    *info = 1;
    ...
    for(k=0; k<=m-1; k++)
    {
         ...
         */
        ae_v_move(&bc.ptr.p_float[0], 1, &b->ptr.pp_float[0][k], b->stride, ae_v_len(0,n-1));
        ...
        for(i=0; i<=n-1; i++)
        {
            mxb = ae_maxreal(mxb, ae_fabs(bc.ptr.p_float[i], _state), _state);
        }
        if( ae_fp_eq(mxb,(float)(0)) )
        {
            mxb = (float)(1);
        }
        scaleright = 1/mxb;
         ...
         */
        ae_v_moved(&xc.ptr.p_float[0], 1, &bc.ptr.p_float[0], 1, ae_v_len(0,n-1), scaleright);
        ...
        if( havea )
        {
            ...
            for(rfs=0; rfs<=nrfs-1; rfs++)
            {
                 ...
                 */
                smallerr = ae_true;
                ae_v_move(&xb.ptr.p_float[0], 1, &xc.ptr.p_float[0], 1, ae_v_len(0,n-1));
                for(i=0; i<=n-1; i++)
                {
                    ae_v_moved(&xa.ptr.p_float[0], 1, &a->ptr.pp_float[i][0], 1, ae_v_len(0,n-1), scalea);
                    xa.ptr.p_float[n] = (float)(-1);
                    xb.ptr.p_float[n] = scaleright*bc.ptr.p_float[i];
                ...
                ae_v_add(&xc.ptr.p_float[0], 1, &y.ptr.p_float[0], 1, ae_v_len(0,n-1));
            }
        }
         ...
         */
        v = scalea*mxb;
        ae_v_moved(&x->ptr.pp_float[0][k], x->stride, &xc.ptr.p_float[0], 1, ae_v_len(0,n-1), v);
    }
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
    ...
    *info = 0;
    ...
    ae_assert(ae_fp_greater(sqrtscalea,(float)(0)), "Assertion failed", _state);
     ...
     ae_state *_state)
{
    ...
    result = densesolver_densesolverrfsmax(n, (float)(0), (float)(0), _state);
    return result;
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        ...
        {
            v = xb->ptr.p_float[i];
            xb->ptr.p_float[i] = xb->ptr.p_float[p->ptr.p_int[i]];
            xb->ptr.p_float[p->ptr.p_int[i]] = v;
        }
    }
    for(i=1; i<=n-1; i++)
    {
        v = ae_v_dotproduct(&lua->ptr.pp_float[i][0], 1, &xb->ptr.p_float[0], 1, ae_v_len(0,i-1));
        xb->ptr.p_float[i] = xb->ptr.p_float[i]-v;
    }
    xb->ptr.p_float[n-1] = xb->ptr.p_float[n-1]/(scalea*lua->ptr.pp_float[n-1][n-1]);
    for(i=n-2; i>=0; i--)
    {
        ae_v_moved(&tmp->ptr.p_float[i+1], 1, &lua->ptr.pp_float[i][i+1], 1, ae_v_len(i+1,n-1), scalea);
        v = ae_v_dotproduct(&tmp->ptr.p_float[i+1], 1, &xb->ptr.p_float[i+1], 1, ae_v_len(i+1,n-1));
        xb->ptr.p_float[i] = (xb->ptr.p_float[i]-v)/(scalea*lua->ptr.pp_float[i][i]);
    }
}
     ...
     ae_state *_state)
{
    ...
    if( isupper )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            xb->ptr.p_float[i] = xb->ptr.p_float[i]/(sqrtscalea*cha->ptr.pp_float[i][i]);
            if( i<n-1 )
            {
                v = xb->ptr.p_float[i];
                ae_v_moved(&tmp->ptr.p_float[i+1], 1, &cha->ptr.pp_float[i][i+1], 1, ae_v_len(i+1,n-1), sqrtscalea);
                ae_v_subd(&xb->ptr.p_float[i+1], 1, &tmp->ptr.p_float[i+1], 1, ae_v_len(i+1,n-1), v);
            }
        }
    ...
    else
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            if( i>0 )
            {
                ae_v_moved(&tmp->ptr.p_float[0], 1, &cha->ptr.pp_float[i][0], 1, ae_v_len(0,i-1), sqrtscalea);
                v = ae_v_dotproduct(&tmp->ptr.p_float[0], 1, &xb->ptr.p_float[0], 1, ae_v_len(0,i-1));
                xb->ptr.p_float[i] = xb->ptr.p_float[i]-v;
            }
            xb->ptr.p_float[i] = xb->ptr.p_float[i]/(sqrtscalea*cha->ptr.pp_float[i][i]);
        }
        ...
        for(i=n-1; i>=0; i--)
        {
            ...
            if( i>0 )
            {
                v = xb->ptr.p_float[i];
                ae_v_moved(&tmp->ptr.p_float[0], 1, &cha->ptr.pp_float[i][0], 1, ae_v_len(0,i-1), sqrtscalea);
                ae_v_subd(&xb->ptr.p_float[0], 1, &tmp->ptr.p_float[0], 1, ae_v_len(0,i-1), v);
            }
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        state->rx.ptr.p_float[i] = _state->v_nan;
    }
    for(i=0; i<=m-1; i++)
    {
        state->b.ptr.p_float[i] = (float)(0);
    }
     ...
     ae_state *_state)
{
    ...
    state->bnorm2 = (float)(0);
    for(i=0; i<=state->m-1; i++)
    {
        state->b.ptr.p_float[i] = b->ptr.p_float[i];
        state->bnorm2 = state->bnorm2+b->ptr.p_float[i]*b->ptr.p_float[i];
    }
}
     ...
     ae_state *_state)
{
    ...
    ae_assert(ae_isfinite(lambdai, _state)&&ae_fp_greater_eq(lambdai,(float)(0)), "LinLSQRSetLambdaI: LambdaI is infinite or NaN", _state);
...
ae_bool linlsqriteration(linlsqrstate* state, ae_state *_state)
{
    ...
    if( state->rstate.stage>=0 )
    {
        ...
        bnorm = state->rstate.ra.ptr.p_float[0];
    }
    else
    {
        summn = -983;
...
lbl_7:
    ...
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->nes.x.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
...
lbl_0:
    ...
    ae_v_move(&state->nes.mv.ptr.p_float[0], 1, &state->mv.ptr.p_float[0], 1, ae_v_len(0,state->m-1));
    goto lbl_7;
lbl_9:
    ...
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->nes.x.ptr.p_float[0], 1, ae_v_len(0,state->m-1));
...
lbl_1:
    ...
    ae_v_move(&state->nes.mtv.ptr.p_float[0], 1, &state->mtv.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    goto lbl_7;
lbl_11:
    goto lbl_7;
lbl_8:
    ...
    {
        state->rx.ptr.p_float[i] = (float)(0);
    }
    ...
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->rx.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    linlsqr_clearrfields(state, _state);
...
lbl_13:
     ...
     */
    state->betai = bnorm;
    if( ae_fp_eq(state->betai,(float)(0)) )
    {
    ...
    for(i=0; i<=summn-1; i++)
    {
        if( i<state->m )
        {
            state->ui.ptr.p_float[i] = state->b.ptr.p_float[i]/state->betai;
        }
        else
        {
            state->ui.ptr.p_float[i] = (float)(0);
        }
        state->x.ptr.p_float[i] = state->ui.ptr.p_float[i];
    }
    state->repnmv = state->repnmv+1;
    linlsqr_clearrfields(state, _state);
    state->needmtv = ae_true;
...
lbl_3:
    ...
    for(i=0; i<=state->n-1; i++)
    {
        state->mtv.ptr.p_float[i] = state->mtv.ptr.p_float[i]+state->lambdai*state->ui.ptr.p_float[state->m+i];
    }
    state->alphai = (float)(0);
    for(i=0; i<=state->n-1; i++)
    {
        state->alphai = state->alphai+state->mtv.ptr.p_float[i]*state->mtv.ptr.p_float[i];
    }
    state->alphai = ae_sqrt(state->alphai, _state);
    if( ae_fp_eq(state->alphai,(float)(0)) )
    {
    ...
    for(i=0; i<=state->n-1; i++)
    {
        state->vi.ptr.p_float[i] = state->mtv.ptr.p_float[i]/state->alphai;
        state->omegai.ptr.p_float[i] = state->vi.ptr.p_float[i];
    }
    state->phibari = state->betai;
    state->rhobari = state->alphai;
    for(i=0; i<=state->n-1; i++)
    {
        state->d.ptr.p_float[i] = (float)(0);
    }
    state->dnorm = (float)(0);
...
lbl_15:
     ...
     */
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->vi.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    state->repnmv = state->repnmv+1;
    linlsqr_clearrfields(state, _state);
    state->needmv = ae_true;
...
lbl_4:
    ...
    for(i=0; i<=state->n-1; i++)
    {
        state->mv.ptr.p_float[state->m+i] = state->lambdai*state->vi.ptr.p_float[i];
    }
    state->betaip1 = (float)(0);
    for(i=0; i<=summn-1; i++)
    {
        state->uip1.ptr.p_float[i] = state->mv.ptr.p_float[i]-state->alphai*state->ui.ptr.p_float[i];
        state->betaip1 = state->betaip1+state->uip1.ptr.p_float[i]*state->uip1.ptr.p_float[i];
    }
    if( ae_fp_neq(state->betaip1,(float)(0)) )
    {
        state->betaip1 = ae_sqrt(state->betaip1, _state);
        for(i=0; i<=summn-1; i++)
        {
            state->uip1.ptr.p_float[i] = state->uip1.ptr.p_float[i]/state->betaip1;
        }
    }
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->uip1.ptr.p_float[0], 1, ae_v_len(0,state->m-1));
    state->repnmv = state->repnmv+1;
    linlsqr_clearrfields(state, _state);
    state->needmtv = ae_true;
...
lbl_5:
    ...
    for(i=0; i<=state->n-1; i++)
    {
        state->mtv.ptr.p_float[i] = state->mtv.ptr.p_float[i]+state->lambdai*state->uip1.ptr.p_float[state->m+i];
    }
    state->alphaip1 = (float)(0);
    for(i=0; i<=state->n-1; i++)
    {
        state->vip1.ptr.p_float[i] = state->mtv.ptr.p_float[i]-state->betaip1*state->vi.ptr.p_float[i];
        state->alphaip1 = state->alphaip1+state->vip1.ptr.p_float[i]*state->vip1.ptr.p_float[i];
    }
    if( ae_fp_neq(state->alphaip1,(float)(0)) )
    {
        state->alphaip1 = ae_sqrt(state->alphaip1, _state);
        for(i=0; i<=state->n-1; i++)
        {
            state->vip1.ptr.p_float[i] = state->vip1.ptr.p_float[i]/state->alphaip1;
        }
    }
    ...
    for(i=0; i<=state->n-1; i++)
    {
        state->d.ptr.p_float[i] = 1/state->rhoi*(state->vi.ptr.p_float[i]-state->theta*state->d.ptr.p_float[i]);
        state->dnorm = state->dnorm+state->d.ptr.p_float[i]*state->d.ptr.p_float[i];
    }
    ...
    for(i=0; i<=state->n-1; i++)
    {
        state->rx.ptr.p_float[i] = state->rx.ptr.p_float[i]+state->phii/state->rhoi*state->omegai.ptr.p_float[i];
    }
    if( !state->xrep )
    {
        goto lbl_17;
...
lbl_17:
    ...
    for(i=0; i<=state->n-1; i++)
    {
        state->omegaip1.ptr.p_float[i] = state->vip1.ptr.p_float[i]-state->theta/state->rhoi*state->omegai.ptr.p_float[i];
    }
     ...
     * ...
     */
    ae_v_move(&state->ui.ptr.p_float[0], 1, &state->uip1.ptr.p_float[0], 1, ae_v_len(0,summn-1));
    ae_v_move(&state->vi.ptr.p_float[0], 1, &state->vip1.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    ae_v_move(&state->omegai.ptr.p_float[0], 1, &state->omegaip1.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
...
lbl_rcomm:
    ...
    state->rstate.ra.ptr.p_float[0] = bnorm;
    return result;
}
     ...
     ae_state *_state)
{
    ...
    if( state->prectype==0 )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            state->tmpd.ptr.p_float[i] = (float)(0);
        }
        t0 = 0;
        t1 = 0;
        while(sparseenumerate(a, &t0, &t1, &i, &j, &v, _state))
        {
            state->tmpd.ptr.p_float[j] = state->tmpd.ptr.p_float[j]+ae_sqr(v, _state);
        }
        for(i=0; i<=n-1; i++)
        {
            if( ae_fp_greater(state->tmpd.ptr.p_float[i],(float)(0)) )
            {
                state->tmpd.ptr.p_float[i] = 1/ae_sqrt(state->tmpd.ptr.p_float[i], _state);
            }
            else
            {
                state->tmpd.ptr.p_float[i] = (float)(1);
            }
        }
    }
    else
    ...
    while(linlsqriteration(state, _state))
    {
        if( state->needmv )
        {
            for(i=0; i<=n-1; i++)
            {
                state->tmpx.ptr.p_float[i] = state->tmpd.ptr.p_float[i]*state->x.ptr.p_float[i];
            }
        ...
        if( state->needmtv )
        {
            ...
            for(i=0; i<=n-1; i++)
            {
                state->mtv.ptr.p_float[i] = state->tmpd.ptr.p_float[i]*state->mtv.ptr.p_float[i];
            }
        }
    }
    for(i=0; i<=n-1; i++)
    {
        state->rx.ptr.p_float[i] = state->tmpd.ptr.p_float[i]*state->rx.ptr.p_float[i];
    }
}
     ...
     ae_state *_state)
{
    ...
    ae_assert(ae_isfinite(epsa, _state)&&ae_fp_greater_eq(epsa,(float)(0)), "LinLSQRSetCond: EpsA is negative, INF or NAN", _state);
    ae_assert(ae_isfinite(epsb, _state)&&ae_fp_greater_eq(epsb,(float)(0)), "LinLSQRSetCond: EpsB is negative, INF or NAN", _state);
    ae_assert(maxits>=0, "LinLSQRSetCond: MaxIts is negative", _state);
    if( (ae_fp_eq(epsa,(float)(0))&&ae_fp_eq(epsb,(float)(0)))&&maxits==0 )
    {
        state->epsa = linlsqr_atol;
        state->epsb = linlsqr_btol;
     ...
     ae_state *_state)
{
    ...
    ae_v_move(&x->ptr.p_float[0], 1, &state->rx.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
...
void lincgcreate(ae_int_t n, lincgstate* state, ae_state *_state)
{
    ...
    {
        state->rx.ptr.p_float[i] = _state->v_nan;
        state->startx.ptr.p_float[i] = 0.0;
        state->b.ptr.p_float[i] = (float)(0);
    }
     ...
     ae_state *_state)
{
    ...
    ae_v_move(&state->startx.ptr.p_float[0], 1, &x->ptr.p_float[0], 1, ae_v_len(0,state->n-1));
}
     ...
     ae_state *_state)
{
    ...
    ae_v_move(&state->b.ptr.p_float[0], 1, &b->ptr.p_float[0], 1, ae_v_len(0,state->n-1));
}
     ...
     ae_state *_state)
{
    ...
    ae_assert(ae_isfinite(epsf, _state)&&ae_fp_greater_eq(epsf,(float)(0)), "LinCGSetCond: EpsF is negative or contains infinite or NaN values", _state);
    ae_assert(maxits>=0, "LinCGSetCond: MaxIts is negative", _state);
    if( ae_fp_eq(epsf,(float)(0))&&maxits==0 )
    {
        state->epsf = lincg_defaultprecision;
        state->maxits = maxits;
    }
...
ae_bool lincgiteration(lincgstate* state, ae_state *_state)
{
    ...
    if( state->rstate.stage>=0 )
    {
        ...
        uvar = state->rstate.ra.ptr.p_float[0];
        bnorm = state->rstate.ra.ptr.p_float[1];
        v = state->rstate.ra.ptr.p_float[2];
    }
    else
    {
        i = -983;
     ...
     */
    ae_v_move(&state->rx.ptr.p_float[0], 1, &state->startx.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->rx.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    state->repnmv = state->repnmv+1;
    lincg_clearrfields(state, _state);
...
lbl_0:
    ...
    bnorm = (float)(0);
    state->r2 = (float)(0);
    state->meritfunction = (float)(0);
    for(i=0; i<=state->n-1; i++)
    {
        state->r.ptr.p_float[i] = state->b.ptr.p_float[i]-state->mv.ptr.p_float[i];
        state->r2 = state->r2+state->r.ptr.p_float[i]*state->r.ptr.p_float[i];
        state->meritfunction = state->meritfunction+state->mv.ptr.p_float[i]*state->rx.ptr.p_float[i]-2*state->b.ptr.p_float[i]*state->rx.ptr.p_float[i];
        bnorm = bnorm+state->b.ptr.p_float[i]*state->b.ptr.p_float[i];
    }
...
lbl_8:
     ...
     * Calculate Z and P
     */
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->r.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    state->repnmv = state->repnmv+1;
    lincg_clearrfields(state, _state);
...
lbl_2:
    ...
    for(i=0; i<=state->n-1; i++)
    {
        state->z.ptr.p_float[i] = state->pv.ptr.p_float[i];
        state->p.ptr.p_float[i] = state->z.ptr.p_float[i];
    }
...
lbl_10:
     ...
     * Calculate Alpha
     */
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->p.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    state->repnmv = state->repnmv+1;
    lincg_clearrfields(state, _state);
    state->needvmv = ae_true;
    state->rstate.stage = 3;
...
lbl_3:
    state->needvmv = ae_false;
    if( !ae_isfinite(state->vmv, _state)||ae_fp_less_eq(state->vmv,(float)(0)) )
    {
    ...
    for(i=0; i<=state->n-1; i++)
    {
        state->alpha = state->alpha+state->r.ptr.p_float[i]*state->z.ptr.p_float[i];
    }
    ...
    for(i=0; i<=state->n-1; i++)
    {
        state->cx.ptr.p_float[i] = state->rx.ptr.p_float[i]+state->alpha*state->p.ptr.p_float[i];
    }
    ...
    for(i=0; i<=state->n-1; i++)
    {
        state->cr.ptr.p_float[i] = state->r.ptr.p_float[i]-state->alpha*state->mv.ptr.p_float[i];
        state->x.ptr.p_float[i] = state->cr.ptr.p_float[i];
    }
    goto lbl_13;
lbl_12:
     ...
     */
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->cx.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    state->repnmv = state->repnmv+1;
    lincg_clearrfields(state, _state);
    state->needmv = ae_true;
    state->rstate.stage = 4;
...
lbl_4:
    ...
    for(i=0; i<=state->n-1; i++)
    {
        state->cr.ptr.p_float[i] = state->b.ptr.p_float[i]-state->mv.ptr.p_float[i];
        state->x.ptr.p_float[i] = state->cr.ptr.p_float[i];
    }
    ...
    for(i=0; i<=state->n-1; i++)
    {
        v = v+state->mv.ptr.p_float[i]*state->cx.ptr.p_float[i]-2*state->b.ptr.p_float[i]*state->cx.ptr.p_float[i];
    }
    ...
    for(i=0; i<=state->n-1; i++)
    {
        if( !ae_isfinite(state->rx.ptr.p_float[i], _state) )
        {
            state->running = ae_false;
            state->repterminationtype = -4;
            result = ae_false;
...
lbl_13:
    ae_v_move(&state->rx.ptr.p_float[0], 1, &state->cx.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    ...
    for(i=0; i<=state->n-1; i++)
    {
        state->r2 = state->r2+state->cr.ptr.p_float[i]*state->cr.ptr.p_float[i];
    }
...
lbl_18:
    ...
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->cr.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
...
lbl_7:
    state->needprec = ae_false;
    ae_v_move(&state->cz.ptr.p_float[0], 1, &state->pv.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    if( state->repiterationscount%state->itsbeforerestart!=0 )
    {
        ...
        for(i=0; i<=state->n-1; i++)
        {
            state->beta = state->beta+state->cz.ptr.p_float[i]*state->cr.ptr.p_float[i];
            uvar = uvar+state->z.ptr.p_float[i]*state->r.ptr.p_float[i];
        }
        
        /*
         *check that UVar is't INF or is't zero
         */
        if( !ae_isfinite(uvar, _state)||ae_fp_eq(uvar,(float)(0)) )
        {
            state->running = ae_false;
            state->repterminationtype = -4;
            result = ae_false;
        ...
        for(i=0; i<=state->n-1; i++)
        {
            state->p.ptr.p_float[i] = state->cz.ptr.p_float[i]+state->beta*state->p.ptr.p_float[i];
        }
    }
    else
    {
        ae_v_move(&state->p.ptr.p_float[0], 1, &state->cz.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    }
    ...
    for(i=0; i<=state->n-1; i++)
    {
         ...
         */
        state->r.ptr.p_float[i] = state->cr.ptr.p_float[i];
        state->z.ptr.p_float[i] = state->cz.ptr.p_float[i];
    }
    goto lbl_10;
lbl_11:
    result = ae_false;
...
lbl_rcomm:
    ...
    state->rstate.ra.ptr.p_float[0] = uvar;
    state->rstate.ra.ptr.p_float[1] = bnorm;
    state->rstate.ra.ptr.p_float[2] = v;
    return result;
}
     ...
     ae_state *_state)
{
    ...
    if( state->prectype==0 )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            ...
            if( ae_fp_greater(v,(float)(0)) )
            {
                state->tmpd.ptr.p_float[i] = 1/ae_sqrt(v, _state);
            }
            else
            {
                state->tmpd.ptr.p_float[i] = (float)(1);
    ...
    while(lincgiteration(state, _state))
    {
        ...
        {
            sparsesmv(a, isupper, &state->x, &state->mv, _state);
            vmv = ae_v_dotproduct(&state->x.ptr.p_float[0], 1, &state->mv.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
            state->vmv = vmv;
        }
        if( state->needprec )
        {
            for(i=0; i<=n-1; i++)
            {
                state->pv.ptr.p_float[i] = state->x.ptr.p_float[i]*ae_sqr(state->tmpd.ptr.p_float[i], _state);
            }
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    state->n = n;
    state->m = m;
    nleqsetcond(state, (float)(0), 0, _state);
    nleqsetxrep(state, ae_false, _state);
    nleqsetstpmax(state, (float)(0), _state);
     ...
     ae_state *_state)
{
    ...
    ae_assert(ae_fp_greater_eq(epsf,(float)(0)), "NLEQSetCond: negative EpsF!", _state);
...
function value at the x+stp*d.
  ...
  -- ALGLIB --
     Copyright 20.08.2010 by Bochkanov Sergey
*************************************************************************/
void nleqsetstpmax(nleqstate* state, float stpmax, ae_state *_state)
{


    ae_assert(ae_isfinite(stpmax, _state), "NLEQSetStpMax: StpMax is not finite!", _state);
    ae_assert(ae_fp_greater_eq(stpmax,(float)(0)), "NLEQSetStpMax: StpMax<0!", _state);
    state->stpmax = stpmax;
}
...
ae_bool nleqiteration(nleqstate* state, ae_state *_state)
{
    ...
    if( state->rstate.stage>=0 )
    {
        ...
        lambdaup = state->rstate.ra.ptr.p_float[0];
        lambdadown = state->rstate.ra.ptr.p_float[1];
        lambdav = state->rstate.ra.ptr.p_float[2];
        rho = state->rstate.ra.ptr.p_float[3];
        mu = state->rstate.ra.ptr.p_float[4];
        stepnorm = state->rstate.ra.ptr.p_float[5];
    }
    else
    {
        n = -983;
...
lbl_0:
    ...
    ae_v_move(&state->xbase.ptr.p_float[0], 1, &state->x.ptr.p_float[0], 1, ae_v_len(0,n-1));
    state->fbase = state->f;
...
lbl_2:
    ...
    ae_v_muld(&state->rightpart.ptr.p_float[0], 1, ae_v_len(0,n-1), -1);
...
lbl_9:
    ...
    for(i=0; i<=n-1; i++)
    {
        state->candstep.ptr.p_float[i] = (float)(0);
    }
    ...
    for(i=0; i<=n-1; i++)
    {
        if( ae_fp_neq(state->candstep.ptr.p_float[i],(float)(0)) )
        {
            stepnorm = (float)(1);
            break;
        }
    ...
    if( ae_fp_neq(state->stpmax,(float)(0)) )
    {
     ...
     */
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->xbase.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_addd(&state->x.ptr.p_float[0], 1, &state->candstep.ptr.p_float[0], 1, ae_v_len(0,n-1), stepnorm);
    b = ae_true;
    for(i=0; i<=n-1; i++)
    {
        if( ae_fp_neq(state->x.ptr.p_float[i],state->xbase.ptr.p_float[i]) )
        {
            b = ae_false;
            break;
        }
...
lbl_10:
     ...
     */
    state->fbase = state->f;
    ae_v_addd(&state->xbase.ptr.p_float[0], 1, &state->candstep.ptr.p_float[0], 1, ae_v_len(0,n-1), stepnorm);
    state->repiterationscount = state->repiterationscount+1;
...
lbl_11:
    ...
    if( ae_fp_eq(stepnorm,(float)(0))&&state->repterminationtype==0 )
    {
        state->repterminationtype = -4;
    }
    if( state->repiterationscount>=state->maxits&&state->maxits>0 )
...
lbl_rcomm:
    ...
    state->rstate.ra.ptr.p_float[0] = lambdaup;
    state->rstate.ra.ptr.p_float[1] = lambdadown;
    state->rstate.ra.ptr.p_float[2] = lambdav;
    state->rstate.ra.ptr.p_float[3] = rho;
    state->rstate.ra.ptr.p_float[4] = mu;
    state->rstate.ra.ptr.p_float[5] = stepnorm;
    return result;
}
     ...
     ae_state *_state)
{
    ...
    ae_v_move(&x->ptr.p_float[0], 1, &state->xbase.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    rep->iterationscount = state->repiterationscount;
     ...
     ae_state *_state)
{
    ...
    ae_v_move(&state->x.ptr.p_float[0], 1, &x->ptr.p_float[0], 1, ae_v_len(0,state->n-1));
     ...
     ae_state *_state)
{
    ...
    if( ae_fp_greater(lnnu+ae_log((float)(2), _state),lnmax) )
    {
        return result;
    }
     ...
     ae_state *_state)
{
    ...
    ae_assert(ae_fp_neq(a->ptr.p_float[n],(float)(0)), "PolynomialSolve: A[N]=0", _state);
     ...
     */
    nz = 0;
    while(nz<n&&ae_fp_eq(a->ptr.p_float[nz],(float)(0)))
    {
        nz = nz+1;
    }
    ne = n-nz;
    for(i=nz; i<=n; i++)
    {
        a->ptr.p_float[i-nz] = a->ptr.p_float[i]/a->ptr.p_float[n];
    }
    ...
    if( ne>0 )
    {
        ...
        c.ptr.pp_float[0][ne-1] = -a->ptr.p_float[0];
        for(i=1; i<=ne-1; i++)
        {
            c.ptr.pp_float[i][i-1] = (float)(1);
            c.ptr.pp_float[i][ne-1] = -a->ptr.p_float[i];
        }
        status = rmatrixevd(&c, ne, 0, &wr, &wi, &vl, &vr, _state);
        ae_assert(status, "PolynomialSolve: inernal error - EVD solver failed", _state);
        for(i=0; i<=ne-1; i++)
        {
            x->ptr.p_complex[i].x = wr.ptr.p_float[i];
            x->ptr.p_complex[i].y = wi.ptr.p_float[i];
        }
    }
    ...
    for(i=0; i<=ne-1; i++)
    {
        ...
        for(j=0; j<=ne; j++)
        {
            v = ae_c_add(v,ae_c_mul_d(vv,a->ptr.p_float[j]));
