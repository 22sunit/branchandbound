...
*************************************************************************/
void odesolverrkck(const real_1d_array &y, const ae_int_t n, const real_1d_array &x, const ae_int_t m, const float eps, const float h, odesolverstate &state)
{
...
SEE ALSO
  ...
  -- ALGLIB --
     Copyright 01.09.2009 by Bochkanov Sergey
*************************************************************************/
void odesolverrkck(const real_1d_array &y, const real_1d_array &x, const float eps, const float h, odesolverstate &state)
{
    alglib_impl::ae_state _alglib_env_state;    
    ae_int_t n;
    ae_int_t m;
...
void odesolversolve(odesolverstate &state,
    void (*diff)(const real_1d_array &y, float x, real_1d_array &dy, void *ptr),
    void *ptr){
    alglib_impl::ae_state _alglib_env_state;
    if( diff==NULL )
...
{
static float odesolver_odesolvermaxgrow = 3.0;
static float odesolver_odesolvermaxshrink = 10.0;
     ...
     ae_state *_state)
{
    ...
    ae_assert(ae_fp_neq(eps,(float)(0)), "ODESolverRKCK: Eps is zero!", _state);
...
ae_bool odesolveriteration(odesolverstate* state, ae_state *_state)
{
    ...
    if( state->rstate.stage>=0 )
    {
        ...
        xc = state->rstate.ra.ptr.p_float[0];
        v = state->rstate.ra.ptr.p_float[1];
        h = state->rstate.ra.ptr.p_float[2];
        h2 = state->rstate.ra.ptr.p_float[3];
        err = state->rstate.ra.ptr.p_float[4];
        maxgrowpow = state->rstate.ra.ptr.p_float[5];
    }
    else
    {
        n = -983;
    ...
    m = state->m;
    h = state->h;
    maxgrowpow = ae_pow(odesolver_odesolvermaxgrow, (float)(5), _state);
    state->repnfev = 0;
     ...
     */
    ae_assert(ae_fp_greater(state->h,(float)(0)), "ODESolver: internal error", _state);
    ...
    state->rka.ptr.p_float[0] = (float)(0);
    state->rka.ptr.p_float[1] = (float)1/(float)5;
    state->rka.ptr.p_float[2] = (float)3/(float)10;
    state->rka.ptr.p_float[3] = (float)3/(float)5;
    state->rka.ptr.p_float[4] = (float)(1);
    state->rka.ptr.p_float[5] = (float)7/(float)8;
    ae_matrix_set_length(&state->rkb, 6, 5, _state);
    state->rkb.ptr.pp_float[1][0] = (float)1/(float)5;
    state->rkb.ptr.pp_float[2][0] = (float)3/(float)40;
    state->rkb.ptr.pp_float[2][1] = (float)9/(float)40;
    state->rkb.ptr.pp_float[3][0] = (float)3/(float)10;
    state->rkb.ptr.pp_float[3][1] = -(float)9/(float)10;
    state->rkb.ptr.pp_float[3][2] = (float)6/(float)5;
    state->rkb.ptr.pp_float[4][0] = -(float)11/(float)54;
    state->rkb.ptr.pp_float[4][1] = (float)5/(float)2;
    state->rkb.ptr.pp_float[4][2] = -(float)70/(float)27;
    state->rkb.ptr.pp_float[4][3] = (float)35/(float)27;
    state->rkb.ptr.pp_float[5][0] = (float)1631/(float)55296;
    state->rkb.ptr.pp_float[5][1] = (float)175/(float)512;
    state->rkb.ptr.pp_float[5][2] = (float)575/(float)13824;
    state->rkb.ptr.pp_float[5][3] = (float)44275/(float)110592;
    state->rkb.ptr.pp_float[5][4] = (float)253/(float)4096;
    ae_vector_set_length(&state->rkc, 6, _state);
    state->rkc.ptr.p_float[0] = (float)37/(float)378;
    state->rkc.ptr.p_float[1] = (float)(0);
    state->rkc.ptr.p_float[2] = (float)250/(float)621;
    state->rkc.ptr.p_float[3] = (float)125/(float)594;
    state->rkc.ptr.p_float[4] = (float)(0);
    state->rkc.ptr.p_float[5] = (float)512/(float)1771;
    ae_vector_set_length(&state->rkcs, 6, _state);
    state->rkcs.ptr.p_float[0] = (float)2825/(float)27648;
    state->rkcs.ptr.p_float[1] = (float)(0);
    state->rkcs.ptr.p_float[2] = (float)18575/(float)48384;
    state->rkcs.ptr.p_float[3] = (float)13525/(float)55296;
    state->rkcs.ptr.p_float[4] = (float)277/(float)14336;
    state->rkcs.ptr.p_float[5] = (float)1/(float)4;
    ...
    xc = state->xg.ptr.p_float[0];
    ae_v_move(&state->ytbl.ptr.pp_float[0][0], 1, &state->yc.ptr.p_float[0], 1, ae_v_len(0,n-1));
    for(j=0; j<=n-1; j++)
    {
        state->escale.ptr.p_float[j] = (float)(0);
    }
    i = 1;
lbl_3:
    if( i>m-1 )
...
lbl_6:
     ...
     */
    if( ae_fp_greater_eq(xc+h,state->xg.ptr.p_float[i]) )
    {
        h = state->xg.ptr.p_float[i]-xc;
        gridpoint = ae_true;
    }
    else
    {
    ...
    for(j=0; j<=n-1; j++)
    {
        state->escale.ptr.p_float[j] = ae_maxreal(state->escale.ptr.p_float[j], ae_fabs(state->yc.ptr.p_float[j], _state), _state);
    }
     ...
     */
    ae_v_move(&state->yn.ptr.p_float[0], 1, &state->yc.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_move(&state->yns.ptr.p_float[0], 1, &state->yc.ptr.p_float[0], 1, ae_v_len(0,n-1));
    k = 0;
lbl_8:
    if( k>5 )
    {
     ...
     */
    state->x = state->xscale*(xc+state->rka.ptr.p_float[k]*h);
    ae_v_move(&state->y.ptr.p_float[0], 1, &state->yc.ptr.p_float[0], 1, ae_v_len(0,n-1));
    for(j=0; j<=k-1; j++)
    {
        v = state->rkb.ptr.pp_float[k][j];
        ae_v_addd(&state->y.ptr.p_float[0], 1, &state->rkk.ptr.pp_float[j][0], 1, ae_v_len(0,n-1), v);
    }
...
lbl_0:
    ...
    v = h*state->xscale;
    ae_v_moved(&state->rkk.ptr.pp_float[k][0], 1, &state->dy.ptr.p_float[0], 1, ae_v_len(0,n-1), v);
    
    /*
     * update YN/YNS
     */
    v = state->rkc.ptr.p_float[k];
    ae_v_addd(&state->yn.ptr.p_float[0], 1, &state->rkk.ptr.pp_float[k][0], 1, ae_v_len(0,n-1), v);
    v = state->rkcs.ptr.p_float[k];
    ae_v_addd(&state->yns.ptr.p_float[0], 1, &state->rkk.ptr.pp_float[k][0], 1, ae_v_len(0,n-1), v);
    k = k+1;
    goto lbl_8;
lbl_10:
    ...
    for(j=0; j<=n-1; j++)
    {
        if( !state->fraceps )
        {
             ...
             */
            err = ae_maxreal(err, ae_fabs(state->yn.ptr.p_float[j]-state->yns.ptr.p_float[j], _state), _state);
        }
        else
        {
             ...
             */
            v = state->escale.ptr.p_float[j];
            if( ae_fp_eq(v,(float)(0)) )
            {
                v = (float)(1);
            }
            err = ae_maxreal(err, ae_fabs(state->yn.ptr.p_float[j]-state->yns.ptr.p_float[j], _state)/v, _state);
        }
    }
     ...
     */
    xc = xc+h;
    ae_v_move(&state->yc.ptr.p_float[0], 1, &state->yn.ptr.p_float[0], 1, ae_v_len(0,n-1));
...
lbl_7:
     ...
     * save result
     */
    ae_v_move(&state->ytbl.ptr.pp_float[i][0], 1, &state->yc.ptr.p_float[0], 1, ae_v_len(0,n-1));
    i = i+1;
    goto lbl_3;
lbl_5:
...
lbl_rcomm:
    ...
    state->rstate.ra.ptr.p_float[0] = xc;
    state->rstate.ra.ptr.p_float[1] = v;
    state->rstate.ra.ptr.p_float[2] = h;
    state->rstate.ra.ptr.p_float[3] = h2;
    state->rstate.ra.ptr.p_float[4] = err;
    state->rstate.ra.ptr.p_float[5] = maxgrowpow;
    return result;
}
    ...
    *m = 0;
    ...
    if( rep->terminationtype>0 )
    {
        *m = state->m;
        ...
        v = state->xscale;
        ae_v_moved(&xtbl->ptr.p_float[0], 1, &state->xg.ptr.p_float[0], 1, ae_v_len(0,state->m-1), v);
        ae_matrix_set_length(ytbl, state->m, state->n, _state);
        for(i=0; i<=state->m-1; i++)
        {
            ae_v_move(&ytbl->ptr.pp_float[i][0], 1, &state->ytbl.ptr.pp_float[i][0], 1, ae_v_len(0,state->n-1));
        }
    }
    else
    {
     ...
     ae_state *_state)
{
     ...
     * check parameters.
     */
    if( (n<=0||m<1)||ae_fp_eq(eps,(float)(0)) )
    {
        state->repterminationtype = -1;
        return;
    }
    if( ae_fp_less(h,(float)(0)) )
    {
        h = -h;
    }
    ...
    if( m==1 )
    {
        ...
        ae_v_move(&state->ytbl.ptr.pp_float[0][0], 1, &y->ptr.p_float[0], 1, ae_v_len(0,n-1));
        ae_vector_set_length(&state->xg, m, _state);
        ae_v_move(&state->xg.ptr.p_float[0], 1, &x->ptr.p_float[0], 1, ae_v_len(0,m-1));
        return;
    }
     ...
     */
    if( ae_fp_eq(x->ptr.p_float[1],x->ptr.p_float[0]) )
    {
    ...
    for(i=1; i<=m-1; i++)
    {
        if( (ae_fp_greater(x->ptr.p_float[1],x->ptr.p_float[0])&&ae_fp_less_eq(x->ptr.p_float[i],x->ptr.p_float[i-1]))||(ae_fp_less(x->ptr.p_float[1],x->ptr.p_float[0])&&ae_fp_greater_eq(x->ptr.p_float[i],x->ptr.p_float[i-1])) )
        {
            state->repterminationtype = -2;
            return;
        }
     ...
     */
    if( ae_fp_eq(h,(float)(0)) )
    {
        v = ae_fabs(x->ptr.p_float[1]-x->ptr.p_float[0], _state);
        for(i=2; i<=m-1; i++)
        {
            v = ae_minreal(v, ae_fabs(x->ptr.p_float[i]-x->ptr.p_float[i-1], _state), _state);
        }
        h = 0.001*v;
    }
    ...
    state->fraceps = ae_fp_less(eps,(float)(0));
    ae_vector_set_length(&state->xg, m, _state);
    ae_v_move(&state->xg.ptr.p_float[0], 1, &x->ptr.p_float[0], 1, ae_v_len(0,m-1));
    if( ae_fp_greater(x->ptr.p_float[1],x->ptr.p_float[0]) )
    {
        state->xscale = (float)(1);
    }
    else
    {
        state->xscale = (float)(-1);
        ae_v_muld(&state->xg.ptr.p_float[0], 1, ae_v_len(0,m-1), -1);
    }
    ae_vector_set_length(&state->yc, n, _state);
    ae_v_move(&state->yc.ptr.p_float[0], 1, &y->ptr.p_float[0], 1, ae_v_len(0,n-1));
