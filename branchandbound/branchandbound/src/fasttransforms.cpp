...
void fftc1d(/* Complex */ ae_vector* a, ae_int_t n, ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        buf.ptr.p_float[2*i+0] = a->ptr.p_complex[i].x;
        buf.ptr.p_float[2*i+1] = a->ptr.p_complex[i].y;
    }
    ...
    for(i=0; i<=n-1; i++)
    {
        a->ptr.p_complex[i].x = buf.ptr.p_float[2*i+0];
        a->ptr.p_complex[i].y = buf.ptr.p_float[2*i+1];
    }
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
    ...
    if( n==1 )
    {
        ...
        f->ptr.p_complex[0] = ae_complex_from_d(a->ptr.p_float[0]);
        ae_frame_leave(_state);
        return;
    }
    if( n==2 )
    {
        ...
        f->ptr.p_complex[0].x = a->ptr.p_float[0]+a->ptr.p_float[1];
        f->ptr.p_complex[0].y = (float)(0);
        f->ptr.p_complex[1].x = a->ptr.p_float[0]-a->ptr.p_float[1];
        f->ptr.p_complex[1].y = (float)(0);
        ae_frame_leave(_state);
        return;
    }
    ...
    if( n%2==0 )
    {
        ...
        ae_v_move(&buf.ptr.p_float[0], 1, &a->ptr.p_float[0], 1, ae_v_len(0,n-1));
        ...
        {
            idx = 2*(i%n2);
            hn.x = buf.ptr.p_float[idx+0];
            hn.y = buf.ptr.p_float[idx+1];
            idx = 2*((n2-i)%n2);
            hmnc.x = buf.ptr.p_float[idx+0];
            hmnc.y = -buf.ptr.p_float[idx+1];
    ...
    else
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            f->ptr.p_complex[i] = ae_complex_from_d(a->ptr.p_float[i]);
        }
        fftc1d(f, n, _state);
    }
    ae_frame_leave(_state);
     ...
     ae_state *_state)
{
    ...
    ae_assert(f->cnt>=ae_ifloor((float)n/(float)2, _state)+1, "FFTR1DInv: Length(F)<Floor(N/2)+1!", _state);
    ae_assert(ae_isfinite(f->ptr.p_complex[0].x, _state), "FFTR1DInv: F contains infinite or NAN values!", _state);
    for(i=1; i<=ae_ifloor((float)n/(float)2, _state)-1; i++)
    {
    ...
    ae_assert(ae_isfinite(f->ptr.p_complex[ae_ifloor((float)n/(float)2, _state)].x, _state), "FFTR1DInv: F contains infinite or NAN values!", _state);
    if( n%2!=0 )
    {
        ae_assert(ae_isfinite(f->ptr.p_complex[ae_ifloor((float)n/(float)2, _state)].y, _state), "FFTR1DInv: F contains infinite or NAN values!", _state);
    }
    ...
    if( n==1 )
    {
        ...
        a->ptr.p_float[0] = f->ptr.p_complex[0].x;
        ae_frame_leave(_state);
        return;
    }
    ...
    h.ptr.p_float[0] = f->ptr.p_complex[0].x;
    for(i=1; i<=ae_ifloor((float)n/(float)2, _state)-1; i++)
    {
        h.ptr.p_float[i] = f->ptr.p_complex[i].x-f->ptr.p_complex[i].y;
        h.ptr.p_float[n-i] = f->ptr.p_complex[i].x+f->ptr.p_complex[i].y;
    }
    if( n%2==0 )
    {
        h.ptr.p_float[ae_ifloor((float)n/(float)2, _state)] = f->ptr.p_complex[ae_ifloor((float)n/(float)2, _state)].x;
    }
    else
    {
        h.ptr.p_float[ae_ifloor((float)n/(float)2, _state)] = f->ptr.p_complex[ae_ifloor((float)n/(float)2, _state)].x-f->ptr.p_complex[ae_ifloor((float)n/(float)2, _state)].y;
        h.ptr.p_float[ae_ifloor((float)n/(float)2, _state)+1] = f->ptr.p_complex[ae_ifloor((float)n/(float)2, _state)].x+f->ptr.p_complex[ae_ifloor((float)n/(float)2, _state)].y;
    }
    fftr1d(&h, n, &fh, _state);
    for(i=0; i<=n-1; i++)
    {
        a->ptr.p_float[i] = (fh.ptr.p_complex[i].x-fh.ptr.p_complex[i].y)/n;
    }
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
    ...
    if( n==2 )
    {
        x = a->ptr.p_float[0]+a->ptr.p_float[1];
        y = a->ptr.p_float[0]-a->ptr.p_float[1];
        a->ptr.p_float[0] = x;
        a->ptr.p_float[1] = y;
        return;
    }
    ...
    ae_v_move(&buf->ptr.p_float[0], 1, &a->ptr.p_float[0], 1, ae_v_len(0,n-1));
    ftapplyplan(plan, buf, 0, 1, _state);
    a->ptr.p_float[0] = buf->ptr.p_float[0]+buf->ptr.p_float[1];
    for(i=1; i<=n2-1; i++)
    {
        idx = 2*(i%n2);
        hn.x = buf->ptr.p_float[idx+0];
        ...
        a->ptr.p_float[2*i+0] = 0.5*v.x;
        a->ptr.p_float[2*i+1] = 0.5*v.y;
    }
    a->ptr.p_float[1] = buf->ptr.p_float[0]-buf->ptr.p_float[1];
}
     ...
     ae_state *_state)
{
    ...
    if( n==2 )
    {
        x = 0.5*(a->ptr.p_float[0]+a->ptr.p_float[1]);
        y = 0.5*(a->ptr.p_float[0]-a->ptr.p_float[1]);
        a->ptr.p_float[0] = x;
        a->ptr.p_float[1] = y;
        return;
    }
     ...
     */
    n2 = n/2;
    buf->ptr.p_float[0] = a->ptr.p_float[0];
    for(i=1; i<=n2-1; i++)
    {
        x = a->ptr.p_float[2*i+0];
        y = a->ptr.p_float[2*i+1];
        buf->ptr.p_float[i] = x-y;
        buf->ptr.p_float[n-i] = x+y;
    }
    buf->ptr.p_float[n2] = a->ptr.p_float[1];
    fftr1dinternaleven(buf, n, a, plan, _state);
    a->ptr.p_float[0] = buf->ptr.p_float[0]/n;
    t = (float)1/(float)n;
    for(i=1; i<=n2-1; i++)
    {
        x = buf->ptr.p_float[2*i+0];
        y = buf->ptr.p_float[2*i+1];
        a->ptr.p_float[i] = t*(x-y);
        a->ptr.p_float[n-i] = t*(x+y);
    }
    a->ptr.p_float[n2] = buf->ptr.p_float[1]/n;
}
     ...
     ae_state *_state)
{
    ...
    for(i=m; i<=p-1; i++)
    {
        buf.ptr.p_float[2*i+0] = (float)(0);
        buf.ptr.p_float[2*i+1] = (float)(0);
    }
    ae_vector_set_length(&buf2, 2*p, _state);
    for(i=0; i<=n-1; i++)
    {
        buf2.ptr.p_float[2*i+0] = b->ptr.p_complex[i].x;
        buf2.ptr.p_float[2*i+1] = b->ptr.p_complex[i].y;
    }
    for(i=n; i<=p-1; i++)
    {
        buf2.ptr.p_float[2*i+0] = (float)(0);
        buf2.ptr.p_float[2*i+1] = (float)(0);
    }
    ftapplyplan(&plan, &buf, 0, 1, _state);
    ftapplyplan(&plan, &buf2, 0, 1, _state);
    for(i=0; i<=p-1; i++)
    {
        c1.x = buf.ptr.p_float[2*i+0];
        c1.y = buf.ptr.p_float[2*i+1];
        c2.x = buf2.ptr.p_float[2*i+0];
        c2.y = buf2.ptr.p_float[2*i+1];
        c3 = ae_c_div(c1,c2);
        buf.ptr.p_float[2*i+0] = c3.x;
        buf.ptr.p_float[2*i+1] = -c3.y;
    }
    ftapplyplan(&plan, &buf, 0, 1, _state);
    t = (float)1/(float)p;
    ...
    for(i=0; i<=m-n; i++)
    {
        r->ptr.p_complex[i].x = t*buf.ptr.p_float[2*i+0];
        r->ptr.p_complex[i].y = -t*buf.ptr.p_float[2*i+1];
    }
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
    ...
    ae_v_move(&buf.ptr.p_float[0], 1, &a->ptr.p_float[0], 1, ae_v_len(0,m-1));
    for(i=m; i<=p-1; i++)
    {
        buf.ptr.p_float[i] = (float)(0);
    }
    ae_vector_set_length(&buf2, p, _state);
    ae_v_move(&buf2.ptr.p_float[0], 1, &b->ptr.p_float[0], 1, ae_v_len(0,n-1));
    for(i=n; i<=p-1; i++)
    {
        buf2.ptr.p_float[i] = (float)(0);
    }
    ...
    buf.ptr.p_float[0] = buf.ptr.p_float[0]/buf2.ptr.p_float[0];
    buf.ptr.p_float[1] = buf.ptr.p_float[1]/buf2.ptr.p_float[1];
    for(i=1; i<=p/2-1; i++)
    {
        c1.x = buf.ptr.p_float[2*i+0];
        c1.y = buf.ptr.p_float[2*i+1];
    ...
    ae_vector_set_length(r, m-n+1, _state);
    ae_v_move(&r->ptr.p_float[0], 1, &buf.ptr.p_float[0], 1, ae_v_len(0,m-n));
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
    ...
    if( m<n )
    {
        ...
        while(i1<n)
        {
            i2 = ae_minint(i1+m-1, n-1, _state);
            j2 = i2-i1;
            ae_v_add(&buf.ptr.p_float[0], 1, &r->ptr.p_float[i1], 1, ae_v_len(0,j2));
            i1 = i1+m;
        }
     ...
     ae_state *_state)
{
    ...
    if( m<n )
    {
        ...
        while(i1<n)
        {
            i2 = ae_minint(i1+m-1, n-1, _state);
            j2 = i2-i1;
            ae_v_add(&buf.ptr.p_float[0], 1, &b->ptr.p_float[i1], 1, ae_v_len(0,j2));
            i1 = i1+m;
        }
    ...
    if( m%2==0 )
    {
        ...
        ae_vector_set_length(r, m, _state);
        ae_v_move(&r->ptr.p_float[0], 1, &buf.ptr.p_float[0], 1, ae_v_len(0,m-1));
    }
    else
    {
        ...
        for(i=0; i<=ae_ifloor((float)m/(float)2, _state); i++)
        {
     ...
     ae_state *_state)
{
    ...
    if( alg==-1||alg==-2 )
    {
        ...
        {
            p = ptotal-n+1;
            flopcand = ae_iceil((float)m/(float)p, _state)*(2*ftbasegetflopestimate(ptotal, _state)+8*ptotal);
            if( ae_fp_less(flopcand,flopbest) )
            {
                flopbest = flopcand;
                algbest = 2;
    ...
    if( alg==1 )
    {
        if( circular&&ftbaseissmooth(m, _state) )
        {
            ...
            for(i=0; i<=m-1; i++)
            {
                ax = buf.ptr.p_float[2*i+0];
                ay = buf.ptr.p_float[2*i+1];
                bx = buf2.ptr.p_float[2*i+0];
                by = buf2.ptr.p_float[2*i+1];
                tx = ax*bx-ay*by;
                ty = ax*by+ay*bx;
                buf.ptr.p_float[2*i+0] = tx;
                buf.ptr.p_float[2*i+1] = -ty;
            }
            ftapplyplan(&plan, &buf, 0, 1, _state);
            t = (float)1/(float)m;
            ae_vector_set_length(r, m, _state);
        ...
        else
        {
            ...
            if( circular )
            {
                ...
                for(i=m; i<=m+n-2; i++)
                {
                    r->ptr.p_complex[i-m].x = r->ptr.p_complex[i-m].x+t*buf.ptr.p_float[2*i+0];
                    r->ptr.p_complex[i-m].y = r->ptr.p_complex[i-m].y-t*buf.ptr.p_float[2*i+1];
                }
            }
            else
            {
    ...
    if( alg==2 )
    {
        ...
        while(i<=m-1)
        {
            ...
            for(j=0; j<=p-1; j++)
            {
                buf.ptr.p_float[2*j+0] = a->ptr.p_complex[i+j].x;
                buf.ptr.p_float[2*j+1] = a->ptr.p_complex[i+j].y;
            }
            for(j=p; j<=q+n-2; j++)
            {
                buf.ptr.p_float[2*j+0] = (float)(0);
                buf.ptr.p_float[2*j+1] = (float)(0);
            }
            ftapplyplan(&plan, &buf, 0, 1, _state);
            for(j=0; j<=q+n-2; j++)
            {
                ax = buf.ptr.p_float[2*j+0];
                ay = buf.ptr.p_float[2*j+1];
                bx = bbuf.ptr.p_complex[j].x;
                by = bbuf.ptr.p_complex[j].y;
                tx = ax*bx-ay*by;
                ty = ax*by+ay*bx;
                buf.ptr.p_float[2*j+0] = tx;
                buf.ptr.p_float[2*j+1] = -ty;
            }
            ftapplyplan(&plan, &buf, 0, 1, _state);
            t = (float)1/(float)(q+n-1);
            if( circular )
            {
            ...
            for(j=0; j<=j1; j++)
            {
                r->ptr.p_complex[i+j].x = r->ptr.p_complex[i+j].x+buf.ptr.p_float[2*j+0]*t;
                r->ptr.p_complex[i+j].y = r->ptr.p_complex[i+j].y-buf.ptr.p_float[2*j+1]*t;
            }
            for(j=j2; j<=p+n-2; j++)
            {
                r->ptr.p_complex[j-j2].x = r->ptr.p_complex[j-j2].x+buf.ptr.p_float[2*j+0]*t;
                r->ptr.p_complex[j-j2].y = r->ptr.p_complex[j-j2].y-buf.ptr.p_float[2*j+1]*t;
            }
            i = i+p;
        }
        ae_frame_leave(_state);
     ...
     ae_state *_state)
{
    ...
    if( alg<0 )
    {
        ...
        if( alg==-1 )
        {
            if( (circular&&ftbaseissmooth(m, _state))&&m%2==0 )
            {
                 ...
                 * special code for circular convolution of a sequence with a smooth length
                 */
                flopcand = 3*ftbasegetflopestimate(m/2, _state)+(float)(6*m)/(float)2;
                if( ae_fp_less(flopcand,flopbest) )
                {
                    algbest = 1;
                    flopbest = flopcand;
            ...
            else
            {
                ...
                flopcand = 3*ftbasegetflopestimate(p/2, _state)+(float)(6*p)/(float)2;
                if( ae_fp_less(flopcand,flopbest) )
                {
                    algbest = 1;
                    flopbest = flopcand;
        ...
        {
            p = ptotal-n+1;
            flopcand = ae_iceil((float)m/(float)p, _state)*(2*ftbasegetflopestimate(ptotal/2, _state)+1*(ptotal/2));
            if( ae_fp_less(flopcand,flopbest) )
            {
                flopbest = flopcand;
                algbest = 2;
    ...
    if( alg==0 )
    {
        ...
        if( n==1 )
        {
            ae_vector_set_length(r, m, _state);
            v = b->ptr.p_float[0];
            ae_v_moved(&r->ptr.p_float[0], 1, &a->ptr.p_float[0], 1, ae_v_len(0,m-1), v);
            ae_frame_leave(_state);
            return;
        }
        ...
        if( circular )
        {
            ...
            for(i=1; i<=n-1; i++)
            {
                ...
                j1 = m-i;
                j2 = m-1;
                ae_v_addd(&r->ptr.p_float[i1], 1, &a->ptr.p_float[j1], 1, ae_v_len(i1,i2), v);
                i1 = i;
                i2 = m-1;
                j1 = 0;
                j2 = m-i-1;
        ...
        else
        {
            ...
            for(i=0; i<=m+n-2; i++)
            {
                r->ptr.p_float[i] = (float)(0);
            }
            for(i=0; i<=n-1; i++)
            {
                v = b->ptr.p_float[i];
                ae_v_addd(&r->ptr.p_float[i], 1, &a->ptr.p_float[0], 1, ae_v_len(i,i+m-1), v);
            }
        }
        ae_frame_leave(_state);
        return;
    ...
    if( alg==1 )
    {
        ...
        else
        {
            ...
            if( circular )
            {
                ...
                if( n>=2 )
                {
                    ae_v_add(&r->ptr.p_float[0], 1, &buf.ptr.p_float[m], 1, ae_v_len(0,n-2));
                }
            }
            else
            {
                 ...
                 */
                ae_vector_set_length(r, m+n-1, _state);
                ae_v_move(&r->ptr.p_float[0], 1, &buf.ptr.p_float[0], 1, ae_v_len(0,m+n-2));
            }
        }
        ae_frame_leave(_state);
        return;
    ...
    if( alg==2 )
    {
        ...
        {
            p = ae_minint(q, m-i, _state);
            ae_v_move(&buf.ptr.p_float[0], 1, &a->ptr.p_float[i], 1, ae_v_len(0,p-1));
            for(j=p; j<=q+n-2; j++)
            {
                buf.ptr.p_float[j] = (float)(0);
            }
            ...
            for(j=1; j<=(q+n-1)/2-1; j++)
            {
                ax = buf.ptr.p_float[2*j+0];
                ay = buf.ptr.p_float[2*j+1];
                bx = buf2.ptr.p_float[2*j+0];
                by = buf2.ptr.p_float[2*j+1];
                tx = ax*bx-ay*by;
                ty = ax*by+ay*bx;
                buf.ptr.p_float[2*j+0] = tx;
                buf.ptr.p_float[2*j+1] = ty;
            ...
            ae_v_add(&r->ptr.p_float[i], 1, &buf.ptr.p_float[0], 1, ae_v_len(i,i+j1));
            if( p+n-2>=j2 )
            {
                ae_v_add(&r->ptr.p_float[0], 1, &buf.ptr.p_float[j2], 1, ae_v_len(0,p+n-2-j2));
            }
            i = i+p;
        }
        ae_frame_leave(_state);
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=m-1; i++)
    {
        p.ptr.p_float[m-1-i] = pattern->ptr.p_float[i];
    }
    convr1d(&p, m, signal, n, &b, _state);
    ae_vector_set_length(r, m+n-1, _state);
    ae_v_move(&r->ptr.p_float[0], 1, &b.ptr.p_float[m-1], 1, ae_v_len(0,n-1));
    if( m+n-2>=n )
    {
        ae_v_move(&r->ptr.p_float[n], 1, &b.ptr.p_float[0], 1, ae_v_len(n,m+n-2));
    }
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
    ...
    if( m<n )
    {
        ...
        while(i1<n)
        {
            i2 = ae_minint(i1+m-1, n-1, _state);
            j2 = i2-i1;
            ae_v_add(&b.ptr.p_float[0], 1, &pattern->ptr.p_float[i1], 1, ae_v_len(0,j2));
            i1 = i1+m;
        }
    ...
    for(i=0; i<=n-1; i++)
    {
        p.ptr.p_float[n-1-i] = pattern->ptr.p_float[i];
    }
    convr1dcircular(signal, m, &p, n, &b, _state);
    ae_vector_set_length(c, m, _state);
    ae_v_move(&c->ptr.p_float[0], 1, &b.ptr.p_float[n-1], 1, ae_v_len(0,m-n));
    if( m-n+1<=m-1 )
    {
        ae_v_move(&c->ptr.p_float[m-n+1], 1, &b.ptr.p_float[0], 1, ae_v_len(m-n+1,m-1));
    }
    ae_frame_leave(_state);
}
...
void fhtr1d(/* Real    */ ae_vector* a, ae_int_t n, ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        a->ptr.p_float[i] = fa.ptr.p_complex[i].x-fa.ptr.p_complex[i].y;
    }
    ae_frame_leave(_state);
}
...
void fhtr1dinv(/* Real    */ ae_vector* a, ae_int_t n, ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        a->ptr.p_float[i] = a->ptr.p_float[i]/n;
    }
}
