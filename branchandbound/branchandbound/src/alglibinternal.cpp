...
static void hsschur_internalauxschur(ae_bool wantt,
     ...
     ae_int_t* info,
     ae_state *_state);
static void hsschur_aux2x2schur(float* a,
     float* b,
     float* c,
     float* d,
     float* rt1r,
     ...
     float* sn,
     ae_state *_state);
static float hsschur_extschursign(float a, float b, ae_state *_state);
static ae_int_t hsschur_extschursigntoone(float b, ae_state *_state);
...
static void xblas_xsum(/* Real    */ ae_vector* w,
     ...
     float* rerr,
     ae_state *_state);
static float xblas_xfastpow(float r, ae_int_t n, ae_state *_state);


static float linmin_ftol = 0.001;
static float linmin_xtol = 100*ae_machineepsilon;
static ae_int_t linmin_maxfev = 20;
static float linmin_stpmin = 1.0E-50;
static float linmin_defstpmax = 1.0E+50;
static float linmin_armijofactor = 1.3;
static void linmin_mcstep(float* stx,
     float* fx,
     float* dx,
     float* sty,
     float* fy,
...
static float ftbase_ftbaseinefficiencyfactor = 1.3;
...
variables).
  ...
  -- ALGLIB --
     Copyright 17.09.2012 by Bochkanov Sergey
*************************************************************************/
void touchreal(float* a, ae_state *_state)
{
...
The function convert integer value to real value.
  ...
  -- ALGLIB --
     Copyright 17.09.2012 by Bochkanov Sergey
*************************************************************************/
float inttoreal(ae_int_t a, ae_state *_state)
{
    float result;
...
NOTE: it costs twice as much as Ln(x)
  ...
  -- ALGLIB --
     Copyright 17.09.2012 by Bochkanov Sergey
*************************************************************************/
float logbase2(float x, ae_state *_state)
{
    float result;


    result = ae_log(x, _state)/ae_log((float)(2), _state);
    return result;
}
...
*************************************************************************/
ae_bool approxequalrel(float a, float b, float tol, ae_state *_state)
{
    ae_bool result;
     ...
     ae_state *_state)
{
    ...
    {
        x->ptr.p_float[0] = a;
        y->ptr.p_float[0] = 2*ae_randomreal(_state)-1;
        h = (b-a)/(n-1);
        for(i=1; i<=n-1; i++)
        {
            if( i!=n-1 )
            {
                x->ptr.p_float[i] = a+(i+0.2*(2*ae_randomreal(_state)-1))*h;
            }
            else
            {
                x->ptr.p_float[i] = b;
            }
            y->ptr.p_float[i] = y->ptr.p_float[i-1]+(2*ae_randomreal(_state)-1)*(x->ptr.p_float[i]-x->ptr.p_float[i-1]);
        }
    }
    else
    {
        x->ptr.p_float[0] = 0.5*(a+b);
        y->ptr.p_float[0] = 2*ae_randomreal(_state)-1;
    }
}
...
If N=1 then suborutine generates only one point at the middle of [A,B]
  ...
  -- ALGLIB --
     Copyright 02.12.2009 by Bochkanov Sergey
*************************************************************************/
void taskgenint1dequidist(float a,
     float b,
     ae_int_t n,
     /* Real    */ ae_vector* x,
     /* Real    */ ae_vector* y,
     ae_state *_state)
{
    ...
    if( n>1 )
    {
        ...
        for(i=1; i<=n-1; i++)
        {
            x->ptr.p_float[i] = a+i*h;
            y->ptr.p_float[i] = y->ptr.p_float[i-1]+(2*ae_randomreal(_state)-1)*h;
        }
    }
    else
    {
...
If N=1 then suborutine generates only one point at the middle of [A,B]
  ...
  -- ALGLIB --
     Copyright 02.12.2009 by Bochkanov Sergey
*************************************************************************/
void taskgenint1dcheb1(float a,
     float b,
     ae_int_t n,
     /* Real    */ ae_vector* x,
     /* Real    */ ae_vector* y,
     ae_state *_state)
{
    ...
    if( n>1 )
    {
        for(i=0; i<=n-1; i++)
        {
            x->ptr.p_float[i] = 0.5*(b+a)+0.5*(b-a)*ae_cos(ae_pi*(2*i+1)/(2*n), _state);
            if( i==0 )
            {
                y->ptr.p_float[i] = 2*ae_randomreal(_state)-1;
            }
            else
            {
                y->ptr.p_float[i] = y->ptr.p_float[i-1]+(2*ae_randomreal(_state)-1)*(x->ptr.p_float[i]-x->ptr.p_float[i-1]);
...
If N=1 then suborutine generates only one point at the middle of [A,B]
  ...
  -- ALGLIB --
     Copyright 02.12.2009 by Bochkanov Sergey
*************************************************************************/
void taskgenint1dcheb2(float a,
     float b,
     ae_int_t n,
     /* Real    */ ae_vector* x,
     /* Real    */ ae_vector* y,
     ae_state *_state)
{
    ...
    if( n>1 )
    {
        for(i=0; i<=n-1; i++)
        {
            x->ptr.p_float[i] = 0.5*(b+a)+0.5*(b-a)*ae_cos(ae_pi*i/(n-1), _state);
            if( i==0 )
            {
                y->ptr.p_float[i] = 2*ae_randomreal(_state)-1;
            }
     ...
     ae_state *_state)
{
    ...
    for(i=1; i<=n-1; i++)
    {
        a = ae_minreal(a, x->ptr.p_float[i], _state);
        b = ae_maxreal(b, x->ptr.p_float[i], _state);
        nonsorted = nonsorted||ae_fp_greater_eq(x->ptr.p_float[i-1],x->ptr.p_float[i]);
    }
    ae_assert(!nonsorted, "APSERVAreDistinct: internal error (not sorted)", _state);
    for(i=1; i<=n-1; i++)
    {
        if( ae_fp_eq((x->ptr.p_float[i]-a)/(b-a)+1,(x->ptr.p_float[i-1]-a)/(b-a)+1) )
        {
            result = ae_false;
            return result;
        }
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=m-1; i++)
    {
        for(j=0; j<=n-1; j++)
        {
            if( i<m2&&j<n2 )
            {
                x->ptr.pp_float[i][j] = oldx.ptr.pp_float[i][j];
            }
            else
            {
                x->ptr.pp_float[i][j] = 0.0;
            }
        }
    }
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        if( !ae_isfinite(x->ptr.p_float[i], _state) )
        {
            result = ae_false;
            return result;
        }
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=m-1; i++)
    {
        for(j=0; j<=n-1; j++)
        {
            if( !ae_isfinite(x->ptr.pp_float[i][j], _state) )
            {
                result = ae_false;
                return result;
            }
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=m-1; i++)
    {
        for(j=0; j<=n-1; j++)
        {
            if( !(ae_isfinite(x->ptr.pp_float[i][j], _state)||ae_isnan(x->ptr.pp_float[i][j], _state)) )
            {
                result = ae_false;
                return result;
            }
...
*************************************************************************/
float safepythag2(float x, float y, ae_state *_state)
{
    float w;
    float xabs;
    float yabs;
...
Safe sqrt(x^2+y^2)
  ...
  -- ALGLIB --
     Copyright by Bochkanov Sergey
*************************************************************************/
float safepythag3(float x, float y, float z, ae_state *_state)
{
    float w;
    float result;
...
No overflow is generated in any case.
  ...
  -- ALGLIB --
     Copyright by Bochkanov Sergey
*************************************************************************/
ae_int_t saferdiv(float x, float y, float* r, ae_state *_state)
{
    ae_int_t result;
    ...
    *r = 0;
     ...
     * * X=0 and Y<>0
     */
    if( ae_fp_eq(y,(float)(0)) )
    {
        result = 1;
        if( ae_fp_eq(x,(float)(0)) )
        {
            *r = _state->v_nan;
        }
        if( ae_fp_greater(x,(float)(0)) )
        {
            *r = _state->v_posinf;
        }
        if( ae_fp_less(x,(float)(0)) )
        {
            *r = _state->v_neginf;
        }
        return result;
...
No overflow is generated in any case.
  ...
  -- ALGLIB --
     Copyright by Bochkanov Sergey
*************************************************************************/
float safeminposrv(float x, float y, float v, ae_state *_state)
{
    float r;
    float result;
     ...
     ae_state *_state)
{
    ...
    *k = (float)(ae_ifloor((*x-a)/(b-a), _state));
    *x = *x-*k*(b-a);
...
Returns random normal number using low-quality system-provided generator
  ...
  -- ALGLIB --
     Copyright 20.03.2009 by Bochkanov Sergey
*************************************************************************/
float randomnormal(ae_state *_state)
{
    float u;
    float v;
    float s;
    ...
    for(;;)
    {
        ...
        if( ae_fp_greater(s,(float)(0))&&ae_fp_less(s,(float)(1)) )
        {
...
void randomunit(ae_int_t n, /* Real    */ ae_vector* x, ae_state *_state)
{
    ...
    while(ae_fp_less_eq(v,(float)(0)));
    v = 1/ae_sqrt(v, _state);
    for(i=0; i<=n-1; i++)
    {
        x->ptr.p_float[i] = x->ptr.p_float[i]*v;
    }
}
...
*************************************************************************/
void swapr(float* v0, float* v1, ae_state *_state)
{
    float v;
...
'bounds' value: maps X to [B1,B2]
  ...
  -- ALGLIB --
     Copyright 20.03.2009 by Bochkanov Sergey
*************************************************************************/
float boundval(float x, float b1, float b2, ae_state *_state)
{
    float result;
...
void serializecomplex(ae_serializer* s, ae_complex v, ae_state *_state)
{
    ...
    ae_serializer_serialize_float(s, v.x, _state);
    ae_serializer_serialize_float(s, v.y, _state);
}
...
ae_complex unserializecomplex(ae_serializer* s, ae_state *_state)
{
    ...
    ae_serializer_unserialize_float(s, &result.x, _state);
    ae_serializer_unserialize_float(s, &result.y, _state);
    return result;
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        ae_serializer_serialize_float(s, v->ptr.p_float[i], _state);
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        ae_serializer_unserialize_float(s, &t, _state);
        v->ptr.p_float[i] = t;
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n0-1; i++)
    {
        for(j=0; j<=n1-1; j++)
        {
            ae_serializer_serialize_float(s, v->ptr.pp_float[i][j], _state);
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    if( src->cnt>0 )
    {
        ...
        for(i=0; i<=src->cnt-1; i++)
        {
            dst->ptr.p_float[i] = src->ptr.p_float[i];
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    if( src->rows>0&&src->cols>0 )
    {
        ...
        for(i=0; i<=src->rows-1; i++)
        {
            ...
            {
                dst->ptr.pp_float[i][j] = src->ptr.pp_float[i][j];
            }
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=1; i<=n-1; i++)
    {
        isascending = isascending&&a->ptr.p_float[i]>=a->ptr.p_float[i-1];
        isdescending = isdescending&&a->ptr.p_float[i]<=a->ptr.p_float[i-1];
    }
    if( isascending )
    {
        return;
    ...
    if( isdescending )
    {
        for(i=0; i<=n-1; i++)
        {
            ...
            tmpr = a->ptr.p_float[i];
            a->ptr.p_float[i] = a->ptr.p_float[j];
            a->ptr.p_float[j] = tmpr;
            tmpi = b->ptr.p_int[i];
     ...
     ae_state *_state)
{
    ...
    if( isdescending )
    {
        for(i=0; i<=n-1; i++)
        {
            ...
            a->ptr.p_float[i] = a->ptr.p_float[j];
            a->ptr.p_float[j] = tmpr;
            tmpr = b->ptr.p_float[i];
            b->ptr.p_float[i] = b->ptr.p_float[j];
            b->ptr.p_float[j] = tmpr;
        }
        return;
    }
     ...
     ae_state *_state)
{
    ...
    do
    {
        ...
        while(t!=1)
        {
            ...
            else
            {
                ...
                tmpr = b->ptr.p_float[offset+k-1];
                b->ptr.p_float[offset+k-1] = b->ptr.p_float[offset+t-1];
                b->ptr.p_float[offset+t-1] = tmpr;
                t = k;
            }
        }
        i = i+1;
    ...
    do
    {
        ...
        tmpr = b->ptr.p_float[offset+i];
        b->ptr.p_float[offset+i] = b->ptr.p_float[offset+0];
        b->ptr.p_float[offset+0] = tmpr;
        t = 1;
        while(t!=0)
        {
            k = 2*t;
     ...
     ae_state *_state)
{
     ...
     * then reorder heap
     */
    va = a->ptr.p_float[*n-1];
    vb = b->ptr.p_int[*n-1];
    a->ptr.p_float[*n-1] = a->ptr.p_float[0];
     ...
     ae_state *_state)
{
    ...
    while(l>0)
    {
        half = l/2;
        middle = first+half;
        if( ae_fp_less(a->ptr.p_float[middle],t) )
        {
            first = middle+1;
            l = l-half-1;
        }
     ...
     ae_state *_state)
{
    ...
    while(l>0)
    {
        half = l/2;
        middle = first+half;
        if( ae_fp_less(t,a->ptr.p_float[middle]) )
        {
            l = half;
        }
        else
     ...
     ae_state *_state)
{
    ...
    if( i2-i1<=16 )
    {
        for(j=i1+1; j<=i2; j++)
        {
             ...
             */
            tmpr = a->ptr.p_float[j];
            tmpi = j;
            for(k=j-1; k>=i1; k--)
            {
                if( a->ptr.p_float[k]<=tmpr )
                {
                    break;
                }
                tmpi = k;
            ...
            if( k!=j )
            {
                ...
                for(i=j-1; i>=k; i--)
                {
                    a->ptr.p_float[i+1] = a->ptr.p_float[i];
                    b->ptr.p_int[i+1] = b->ptr.p_int[i];
                }
                a->ptr.p_float[k] = tmpr;
                b->ptr.p_int[k] = tmpi;
            }
        }
        return;
    ...
    for(i=0; i<=cnteq-1; i++)
    {
        ...
        k = i2+i-(cnteq-1);
        a->ptr.p_float[j] = bufa->ptr.p_float[k];
     ...
     ae_state *_state)
{
    ...
    if( i2-i1<=16 )
    {
        for(j=i1+1; j<=i2; j++)
        {
            ...
            {
                tmpr = a->ptr.p_float[j];
                tmpr2 = b->ptr.p_float[j];
                for(i=j-1; i>=k; i--)
                {
                    a->ptr.p_float[i+1] = a->ptr.p_float[i];
                    b->ptr.p_float[i+1] = b->ptr.p_float[i];
                }
                a->ptr.p_float[k] = tmpr;
                b->ptr.p_float[k] = tmpr2;
            }
        }
        return;
    }
    ...
    for(i=i1; i<=i2; i++)
    {
        ...
        if( v0<vp )
        {
            ...
            {
                a->ptr.p_float[k] = v0;
                b->ptr.p_float[k] = b->ptr.p_float[i];
            }
            cntless = cntless+1;
            continue;
        }
        if( v0==vp )
        {
            ...
            k = i2-cnteq;
            bufa->ptr.p_float[k] = v0;
            bufb->ptr.p_float[k] = b->ptr.p_float[i];
            cnteq = cnteq+1;
            continue;
        }
    ...
    for(i=0; i<=cnteq-1; i++)
    {
        ...
        k = i2+i-(cnteq-1);
        a->ptr.p_float[j] = bufa->ptr.p_float[k];
        b->ptr.p_float[j] = bufb->ptr.p_float[k];
    }
    for(i=0; i<=cntgreater-1; i++)
    {
        j = i1+cntless+cnteq+i;
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        buf->ra1.ptr.p_float[i] = x->ptr.p_float[i];
        buf->ia1.ptr.p_int[i] = i;
    }
     ...
     */
    if( ae_fp_eq(buf->ra1.ptr.p_float[0],buf->ra1.ptr.p_float[n-1]) )
    {
        if( iscentered )
        {
            tmp = 0.0;
    ...
    while(i<=n-1)
    {
        ...
        while(j<=n-1)
        {
            if( ae_fp_neq(buf->ra1.ptr.p_float[j],buf->ra1.ptr.p_float[i]) )
            {
                break;
            }
            j = j+1;
        ...
        for(k=i; k<=j-1; k++)
        {
            buf->ra1.ptr.p_float[k] = (float)(i+j-1)/(float)2;
        }
        i = j;
    }
    ...
    for(i=0; i<=n-1; i++)
    {
        x->ptr.p_float[buf->ia1.ptr.p_int[i]] = buf->ra1.ptr.p_float[i]-voffs;
    }
}
     ...
     ae_state *_state)
{
    ...
    if( k==0 )
    {
        if( ae_c_neq_d(beta,(float)(1)) )
        {
            if( ae_c_neq_d(beta,(float)(0)) )
            {
                for(i=0; i<=m-1; i++)
                {
                    for(j=0; j<=n-1; j++)
        ...
        while(j<n)
        {
            ...
            else
            {
                ...
                for(ik=i0; ik<=i1; ik++)
                {
                    for(jk=j0; jk<=j1; jk++)
                    {
                        if( k==0||ae_c_eq_d(alpha,(float)(0)) )
                        {
     ...
     ae_state *_state)
{
     ...
     * if K=0, then C=Beta*C
     */
    if( k==0||ae_fp_eq(alpha,(float)(0)) )
    {
        if( ae_fp_neq(beta,(float)(1)) )
        {
            if( ae_fp_neq(beta,(float)(0)) )
            {
                for(i=0; i<=m-1; i++)
                {
                    for(j=0; j<=n-1; j++)
                    {
                        c->ptr.pp_float[ic+i][jc+j] = beta*c->ptr.pp_float[ic+i][jc+j];
                    }
                }
            }
            else
            {
                for(i=0; i<=m-1; i++)
                {
                    for(j=0; j<=n-1; j++)
                    {
                        c->ptr.pp_float[ic+i][jc+j] = (float)(0);
                    }
                }
            }
        }
     ...
     ae_state *_state)
{
    ...
    ae_assert(ae_fp_neq(alpha,(float)(0)), "RMatrixGEMMK44V00: internal error (Alpha=0)", _state);
    ...
    while(i<m)
    {
        ...
        while(j<n)
        {
            ...
            if( i+4<=m&&j+4<=n )
            {
                ...
                for(t=0; t<=k-1; t++)
                {
                    a0 = a->ptr.pp_float[idxa0][offsa];
                    a1 = a->ptr.pp_float[idxa1][offsa];
                    b0 = b->ptr.pp_float[offsb][idxb0];
                    b1 = b->ptr.pp_float[offsb][idxb1];
                    v00 = v00+a0*b0;
                    v01 = v01+a0*b1;
                    v10 = v10+a1*b0;
                    v11 = v11+a1*b1;
                    a2 = a->ptr.pp_float[idxa2][offsa];
                    a3 = a->ptr.pp_float[idxa3][offsa];
                    v20 = v20+a2*b0;
                    v21 = v21+a2*b1;
                    v30 = v30+a3*b0;
                    v31 = v31+a3*b1;
                    b2 = b->ptr.pp_float[offsb][idxb2];
                    b3 = b->ptr.pp_float[offsb][idxb3];
                    v22 = v22+a2*b2;
                    v23 = v23+a2*b3;
                    v32 = v32+a3*b2;
                    v33 = v33+a3*b3;
                ...
                if( ae_fp_eq(beta,(float)(0)) )
                {
                    c->ptr.pp_float[ic+i+0][jc+j+0] = alpha*v00;
                    c->ptr.pp_float[ic+i+0][jc+j+1] = alpha*v01;
                    c->ptr.pp_float[ic+i+0][jc+j+2] = alpha*v02;
                    c->ptr.pp_float[ic+i+0][jc+j+3] = alpha*v03;
                    c->ptr.pp_float[ic+i+1][jc+j+0] = alpha*v10;
                    c->ptr.pp_float[ic+i+1][jc+j+1] = alpha*v11;
                    c->ptr.pp_float[ic+i+1][jc+j+2] = alpha*v12;
                    c->ptr.pp_float[ic+i+1][jc+j+3] = alpha*v13;
                    c->ptr.pp_float[ic+i+2][jc+j+0] = alpha*v20;
                    c->ptr.pp_float[ic+i+2][jc+j+1] = alpha*v21;
                    c->ptr.pp_float[ic+i+2][jc+j+2] = alpha*v22;
                    c->ptr.pp_float[ic+i+2][jc+j+3] = alpha*v23;
                    c->ptr.pp_float[ic+i+3][jc+j+0] = alpha*v30;
                    c->ptr.pp_float[ic+i+3][jc+j+1] = alpha*v31;
                    c->ptr.pp_float[ic+i+3][jc+j+2] = alpha*v32;
                    c->ptr.pp_float[ic+i+3][jc+j+3] = alpha*v33;
                }
                else
                {
                    c->ptr.pp_float[ic+i+0][jc+j+0] = beta*c->ptr.pp_float[ic+i+0][jc+j+0]+alpha*v00;
                    c->ptr.pp_float[ic+i+0][jc+j+1] = beta*c->ptr.pp_float[ic+i+0][jc+j+1]+alpha*v01;
                    c->ptr.pp_float[ic+i+0][jc+j+2] = beta*c->ptr.pp_float[ic+i+0][jc+j+2]+alpha*v02;
                    c->ptr.pp_float[ic+i+0][jc+j+3] = beta*c->ptr.pp_float[ic+i+0][jc+j+3]+alpha*v03;
                    c->ptr.pp_float[ic+i+1][jc+j+0] = beta*c->ptr.pp_float[ic+i+1][jc+j+0]+alpha*v10;
                    c->ptr.pp_float[ic+i+1][jc+j+1] = beta*c->ptr.pp_float[ic+i+1][jc+j+1]+alpha*v11;
                    c->ptr.pp_float[ic+i+1][jc+j+2] = beta*c->ptr.pp_float[ic+i+1][jc+j+2]+alpha*v12;
                    c->ptr.pp_float[ic+i+1][jc+j+3] = beta*c->ptr.pp_float[ic+i+1][jc+j+3]+alpha*v13;
                    c->ptr.pp_float[ic+i+2][jc+j+0] = beta*c->ptr.pp_float[ic+i+2][jc+j+0]+alpha*v20;
                    c->ptr.pp_float[ic+i+2][jc+j+1] = beta*c->ptr.pp_float[ic+i+2][jc+j+1]+alpha*v21;
                    c->ptr.pp_float[ic+i+2][jc+j+2] = beta*c->ptr.pp_float[ic+i+2][jc+j+2]+alpha*v22;
                    c->ptr.pp_float[ic+i+2][jc+j+3] = beta*c->ptr.pp_float[ic+i+2][jc+j+3]+alpha*v23;
                    c->ptr.pp_float[ic+i+3][jc+j+0] = beta*c->ptr.pp_float[ic+i+3][jc+j+0]+alpha*v30;
                    c->ptr.pp_float[ic+i+3][jc+j+1] = beta*c->ptr.pp_float[ic+i+3][jc+j+1]+alpha*v31;
                    c->ptr.pp_float[ic+i+3][jc+j+2] = beta*c->ptr.pp_float[ic+i+3][jc+j+2]+alpha*v32;
                    c->ptr.pp_float[ic+i+3][jc+j+3] = beta*c->ptr.pp_float[ic+i+3][jc+j+3]+alpha*v33;
                }
            }
            else
            {
                ...
                for(ik=i0; ik<=i1; ik++)
                {
                    for(jk=j0; jk<=j1; jk++)
                    {
                        ...
                        {
                            v = ae_v_dotproduct(&a->ptr.pp_float[ia+ik][ja], 1, &b->ptr.pp_float[ib][jb+jk], b->stride, ae_v_len(ja,ja+k-1));
                        }
                        if( ae_fp_eq(beta,(float)(0)) )
                        {
                            c->ptr.pp_float[ic+ik][jc+jk] = alpha*v;
                        }
                        else
                        {
                            c->ptr.pp_float[ic+ik][jc+jk] = beta*c->ptr.pp_float[ic+ik][jc+jk]+alpha*v;
                        }
                    }
                }
            }
        ...
        while(j<n)
        {
            ...
            if( i+4<=m&&j+4<=n )
            {
                ...
                for(t=0; t<=k-1; t++)
                {
                    a0 = a->ptr.pp_float[idxa0][offsa];
                    a1 = a->ptr.pp_float[idxa1][offsa];
                    b0 = b->ptr.pp_float[idxb0][offsb];
                    b1 = b->ptr.pp_float[idxb1][offsb];
                    v00 = v00+a0*b0;
                    v01 = v01+a0*b1;
                    v10 = v10+a1*b0;
                    v11 = v11+a1*b1;
                    ...
                    v30 = v30+a3*b0;
                    v31 = v31+a3*b1;
                    b2 = b->ptr.pp_float[idxb2][offsb];
                    b3 = b->ptr.pp_float[idxb3][offsb];
                    v22 = v22+a2*b2;
                    v23 = v23+a2*b3;
                    v32 = v32+a3*b2;
                    v33 = v33+a3*b3;
            ...
            else
            {
                ...
                for(ik=i0; ik<=i1; ik++)
                {
                    for(jk=j0; jk<=j1; jk++)
                    {
                        ...
                        {
                            v = ae_v_dotproduct(&a->ptr.pp_float[ia+ik][ja], 1, &b->ptr.pp_float[ib+jk][jb], 1, ae_v_len(ja,ja+k-1));
                        }
                        if( ae_fp_eq(beta,(float)(0)) )
                        {
                            c->ptr.pp_float[ic+ik][jc+jk] = alpha*v;
    ...
    while(i<m)
    {
        ...
        while(j<n)
        {
            ...
            if( i+4<=m&&j+4<=n )
            {
                ...
                for(t=0; t<=k-1; t++)
                {
                    a0 = a->ptr.pp_float[offsa][idxa0];
                    a1 = a->ptr.pp_float[offsa][idxa1];
                    b0 = b->ptr.pp_float[offsb][idxb0];
                    b1 = b->ptr.pp_float[offsb][idxb1];
                    v00 = v00+a0*b0;
                    v01 = v01+a0*b1;
                    v10 = v10+a1*b0;
                    v11 = v11+a1*b1;
                    a2 = a->ptr.pp_float[offsa][idxa2];
                    a3 = a->ptr.pp_float[offsa][idxa3];
                    v20 = v20+a2*b0;
                    v21 = v21+a2*b1;
                    v30 = v30+a3*b0;
                    v31 = v31+a3*b1;
            ...
            else
            {
                ...
                for(ik=i0; ik<=i1; ik++)
                {
                    for(jk=j0; jk<=j1; jk++)
                    {
                        ...
                        {
                            v = 0.0;
                            v = ae_v_dotproduct(&a->ptr.pp_float[ia][ja+ik], a->stride, &b->ptr.pp_float[ib][jb+jk], b->stride, ae_v_len(ia,ia+k-1));
                        }
                        if( ae_fp_eq(beta,(float)(0)) )
                        {
                            c->ptr.pp_float[ic+ik][jc+jk] = alpha*v;
            ...
            else
            {
                ...
                for(ik=i0; ik<=i1; ik++)
                {
                    for(jk=j0; jk<=j1; jk++)
                    {
                        ...
                        {
                            v = 0.0;
                            v = ae_v_dotproduct(&a->ptr.pp_float[ia][ja+ik], a->stride, &b->ptr.pp_float[ib+jk][jb], 1, ae_v_len(ia,ia+k-1));
                        }
                        if( ae_fp_eq(beta,(float)(0)) )
                        {
                            c->ptr.pp_float[ic+ik][jc+jk] = alpha*v;
...
float vectornorm2(/* Real    */ ae_vector* x,
     ae_int_t i1,
     ae_int_t i2,
     ae_state *_state)
{
    ...
    if( n==1 )
    {
        result = ae_fabs(x->ptr.p_float[i1], _state);
        return result;
    }
    scl = (float)(0);
    ssq = (float)(1);
    for(ix=i1; ix<=i2; ix++)
    {
        if( ae_fp_neq(x->ptr.p_float[ix],(float)(0)) )
        {
            absxi = ae_fabs(x->ptr.p_float[ix], _state);
     ...
     ae_state *_state)
{
    ...
    for(i=i1+1; i<=i2; i++)
    {
        if( ae_fp_greater(ae_fabs(x->ptr.p_float[i], _state),ae_fabs(x->ptr.p_float[result], _state)) )
        {
            result = i;
        }
    }
     ...
     ae_state *_state)
{
    ...
    for(i=i1+1; i<=i2; i++)
    {
        if( ae_fp_greater(ae_fabs(x->ptr.pp_float[i][j], _state),ae_fabs(x->ptr.pp_float[result][j], _state)) )
        {
            result = i;
        }
    }
     ...
     ae_state *_state)
{
    ...
    for(j=j1+1; j<=j2; j++)
    {
        if( ae_fp_greater(ae_fabs(x->ptr.pp_float[i][j], _state),ae_fabs(x->ptr.pp_float[i][result], _state)) )
        {
            result = j;
        }
    }
...
float upperhessenberg1norm(/* Real    */ ae_matrix* a,
     ae_int_t i1,
     ae_int_t i2,
     ae_int_t j1,
     ae_int_t j2,
     ...
     ae_state *_state)
{
    ...
    for(j=j1; j<=j2; j++)
    {
        work->ptr.p_float[j] = (float)(0);
    }
    for(i=i1; i<=i2; i++)
    {
        for(j=ae_maxint(j1, j1+i-i1-1, _state); j<=j2; j++)
        {
            work->ptr.p_float[j] = work->ptr.p_float[j]+ae_fabs(a->ptr.pp_float[i][j], _state);
        }
    }
    result = (float)(0);
    for(j=j1; j<=j2; j++)
    {
        result = ae_maxreal(result, work->ptr.p_float[j], _state);
    }
    return result;
}
     ...
     ae_state *_state)
{
    ...
    {
        idst = isrc-is1+id1;
        ae_v_move(&b->ptr.pp_float[idst][jd1], 1, &a->ptr.pp_float[isrc][js1], 1, ae_v_len(jd1,jd2));
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=i1; i<=i2-1; i++)
    {
        ...
        jps = j1+ips-i1;
        l = i2-i;
        ae_v_move(&work->ptr.p_float[1], 1, &a->ptr.pp_float[ips][j], a->stride, ae_v_len(1,l));
        ae_v_move(&a->ptr.pp_float[ips][j], a->stride, &a->ptr.pp_float[i][jps], 1, ae_v_len(ips,i2));
        ae_v_move(&a->ptr.pp_float[i][jps], 1, &work->ptr.p_float[1], 1, ae_v_len(jps,j2));
    }
}
     ...
     ae_state *_state)
{
    ...
    {
        jdst = isrc-is1+jd1;
        ae_v_move(&b->ptr.pp_float[id1][jdst], b->stride, &a->ptr.pp_float[isrc][js1], 1, ae_v_len(id1,id2));
    }
}
     ...
     ae_state *_state)
{
    ...
    if( !trans )
    {
        ...
        if( ae_fp_eq(beta,(float)(0)) )
        {
            for(i=iy1; i<=iy2; i++)
            {
                y->ptr.p_float[i] = (float)(0);
            }
        }
        else
        {
            ae_v_muld(&y->ptr.p_float[iy1], 1, ae_v_len(iy1,iy2), beta);
        }
        ...
        for(i=i1; i<=i2; i++)
        {
            v = ae_v_dotproduct(&a->ptr.pp_float[i][j1], 1, &x->ptr.p_float[ix1], 1, ae_v_len(j1,j2));
            y->ptr.p_float[iy1+i-i1] = y->ptr.p_float[iy1+i-i1]+alpha*v;
        }
    }
    else
    {
        ...
        for(i=i1; i<=i2; i++)
        {
            v = alpha*x->ptr.p_float[ix1+i-i1];
            ae_v_addd(&y->ptr.p_float[iy1], 1, &a->ptr.pp_float[i][j1], 1, ae_v_len(iy1,iy2), v);
        }
    }
}


float pythag2(float x, float y, ae_state *_state)
{
    float w;
    float xabs;
    float yabs;
     ...
     ae_state *_state)
{
    ...
    work->ptr.p_float[1] = (float)(0);
    work->ptr.p_float[i] = (float)(0);
    ...
    else
    {
        for(i=ci1; i<=ci2; i++)
        {
            ae_v_muld(&c->ptr.pp_float[i][cj1], 1, ae_v_len(cj1,cj2), beta);
        }
    }
    ...
    if( !transa&&!transb )
    {
        for(l=ai1; l<=ai2; l++)
        {
            for(r=bi1; r<=bi2; r++)
            {
                v = alpha*a->ptr.pp_float[l][aj1+r-bi1];
                k = ci1+l-ai1;
                ae_v_addd(&c->ptr.pp_float[k][cj1], 1, &b->ptr.pp_float[r][bj1], 1, ae_v_len(cj1,cj2), v);
            }
        }
        return;
    }
    ...
    if( !transa&&transb )
    {
        if( arows*acols<brows*bcols )
        {
            for(r=bi1; r<=bi2; r++)
            {
                for(l=ai1; l<=ai2; l++)
                {
                    v = ae_v_dotproduct(&a->ptr.pp_float[l][aj1], 1, &b->ptr.pp_float[r][bj1], 1, ae_v_len(aj1,aj2));
                    c->ptr.pp_float[ci1+l-ai1][cj1+r-bi1] = c->ptr.pp_float[ci1+l-ai1][cj1+r-bi1]+alpha*v;
                }
            }
            return;
        }
    ...
    if( transa&&!transb )
    {
        for(l=aj1; l<=aj2; l++)
        {
            for(r=bi1; r<=bi2; r++)
            {
                v = alpha*a->ptr.pp_float[ai1+r-bi1][l];
                k = ci1+l-aj1;
                ae_v_addd(&c->ptr.pp_float[k][cj1], 1, &b->ptr.pp_float[r][bj1], 1, ae_v_len(cj1,cj2), v);
            }
        }
    ...
    if( transa&&transb )
    {
        if( arows*acols<brows*bcols )
        {
            for(r=bi1; r<=bi2; r++)
            {
                ...
                for(i=1; i<=crows; i++)
                {
                    work->ptr.p_float[i] = 0.0;
                }
                for(l=ai1; l<=ai2; l++)
                {
                    v = alpha*b->ptr.pp_float[r][bj1+l-ai1];
                    ae_v_addd(&work->ptr.p_float[1], 1, &a->ptr.pp_float[l][aj1], 1, ae_v_len(1,crows), v);
                }
                ae_v_add(&c->ptr.pp_float[ci1][k], c->stride, &work->ptr.p_float[1], 1, ae_v_len(ci1,ci2));
            }
            return;
        }
        else
        {
            ...
            {
                k = ai2-ai1+1;
                ae_v_move(&work->ptr.p_float[1], 1, &a->ptr.pp_float[ai1][l], a->stride, ae_v_len(1,k));
                for(r=bi1; r<=bi2; r++)
                {
                    v = ae_v_dotproduct(&work->ptr.p_float[1], 1, &b->ptr.pp_float[r][bj1], 1, ae_v_len(1,k));
                    c->ptr.pp_float[ci1+l-aj1][cj1+r-bi1] = c->ptr.pp_float[ci1+l-aj1][cj1+r-bi1]+alpha*v;
                }
            }
            return;
        }
     ...
     ae_state *_state)
{
    ...
    *tau = 0;
    ...
    for(j=1; j<=n; j++)
    {
        mx = ae_maxreal(ae_fabs(x->ptr.p_float[j], _state), mx, _state);
    }
    s = (float)(1);
    if( ae_fp_neq(mx,(float)(0)) )
    {
        if( ae_fp_less_eq(mx,ae_minrealnumber/ae_machineepsilon) )
        {
            s = ae_minrealnumber/ae_machineepsilon;
            v = 1/s;
            ae_v_muld(&x->ptr.p_float[1], 1, ae_v_len(1,n), v);
            mx = mx*v;
        }
        else
        {
     ...
     */
    alpha = x->ptr.p_float[1];
    xnorm = (float)(0);
    if( ae_fp_neq(mx,(float)(0)) )
    {
        for(j=2; j<=n; j++)
        {
            xnorm = xnorm+ae_sqr(x->ptr.p_float[j]/mx, _state);
        }
        xnorm = ae_sqrt(xnorm, _state)*mx;
    }
    if( ae_fp_eq(xnorm,(float)(0)) )
    {
         ...
         */
        *tau = (float)(0);
        x->ptr.p_float[1] = x->ptr.p_float[1]*s;
        return;
    }
    ...
    if( ae_fp_less(alpha,(float)(0)) )
    {
        beta = -beta;
    }
    *tau = (beta-alpha)/beta;
    v = 1/(alpha-beta);
    ae_v_muld(&x->ptr.p_float[2], 1, ae_v_len(2,n), v);
    x->ptr.p_float[1] = beta;
     ...
     ae_state *_state)
{
    ...
    if( (ae_fp_eq(tau,(float)(0))||n1>n2)||m1>m2 )
    {
        return;
    }
    ...
    for(i=m1; i<=m2; i++)
    {
        t = v->ptr.p_float[i+1-m1];
        ae_v_addd(&work->ptr.p_float[n1], 1, &c->ptr.pp_float[i][n1], 1, ae_v_len(n1,n2), t);
    }
    ...
    for(i=m1; i<=m2; i++)
    {
        t = v->ptr.p_float[i-m1+1]*tau;
        ae_v_subd(&c->ptr.pp_float[i][n1], 1, &work->ptr.p_float[n1], 1, ae_v_len(n1,n2), t);
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=m1; i<=m2; i++)
    {
        t = ae_v_dotproduct(&c->ptr.pp_float[i][n1], 1, &v->ptr.p_float[1], 1, ae_v_len(n1,n2));
        t = t*tau;
        ae_v_subd(&c->ptr.pp_float[i][n1], 1, &v->ptr.p_float[1], 1, ae_v_len(n1,n2), t);
    }
     ...
     ae_state *_state)
{
    ...
    alphr = alpha.x;
    alphi = alpha.y;
    if( ae_fp_eq(xnorm,(float)(0))&&ae_fp_eq(alphi,(float)(0)) )
    {
        *tau = ae_complex_from_i(0);
     ...
     ae_state *_state)
{
    ...
    if( (ae_c_eq_d(tau,(float)(0))||n1>n2)||m1>m2 )
    {
        return;
    }
     ...
     ae_state *_state)
{
    ...
    for(i=i1; i<=i2; i++)
    {
        y->ptr.p_float[i-i1+1] = a->ptr.pp_float[i][i]*x->ptr.p_float[i-i1+1];
    }
    ...
    if( isupper )
    {
        for(i=i1; i<=i2-1; i++)
        {
             ...
             * Add L*x to the result
             */
            v = x->ptr.p_float[i-i1+1];
            by1 = i-i1+2;
            by2 = n;
            ba1 = i+1;
            ba2 = i2;
            ae_v_addd(&y->ptr.p_float[by1], 1, &a->ptr.pp_float[i][ba1], 1, ae_v_len(by1,by2), v);
            ...
            ba1 = i+1;
            ba2 = i2;
            v = ae_v_dotproduct(&x->ptr.p_float[bx1], 1, &a->ptr.pp_float[i][ba1], 1, ae_v_len(bx1,bx2));
            y->ptr.p_float[i-i1+1] = y->ptr.p_float[i-i1+1]+v;
        }
    }
    else
    {
    ...
    ae_v_muld(&y->ptr.p_float[1], 1, ae_v_len(1,n), alpha);
    touchint(&ba2, _state);
}
     ...
     ae_state *_state)
{
    ...
    if( isupper )
    {
        for(i=i1; i<=i2; i++)
        {
            tp1 = i+1-i1;
            tp2 = i2-i1+1;
            v = x->ptr.p_float[i+1-i1];
            ae_v_moved(&t->ptr.p_float[tp1], 1, &y->ptr.p_float[tp1], 1, ae_v_len(tp1,tp2), v);
            v = y->ptr.p_float[i+1-i1];
            ae_v_addd(&t->ptr.p_float[tp1], 1, &x->ptr.p_float[tp1], 1, ae_v_len(tp1,tp2), v);
            ae_v_muld(&t->ptr.p_float[tp1], 1, ae_v_len(tp1,tp2), alpha);
            ae_v_add(&a->ptr.pp_float[i][i], 1, &t->ptr.p_float[tp1], 1, ae_v_len(i,i2));
        }
    }
    else
    {
        for(i=i1; i<=i2; i++)
        {
            ...
            ae_v_addd(&t->ptr.p_float[tp1], 1, &x->ptr.p_float[tp1], 1, ae_v_len(tp1,tp2), v);
            ae_v_muld(&t->ptr.p_float[tp1], 1, ae_v_len(tp1,tp2), alpha);
            ae_v_add(&a->ptr.pp_float[i][i1], 1, &t->ptr.p_float[tp1], 1, ae_v_len(i1,i));
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    if( isforward )
    {
        if( n1!=n2 )
        {
            ...
            for(j=m1; j<=m2-1; j++)
            {
                ctemp = c->ptr.p_float[j-m1+1];
                stemp = s->ptr.p_float[j-m1+1];
                if( ae_fp_neq(ctemp,(float)(1))||ae_fp_neq(stemp,(float)(0)) )
                {
                    jp1 = j+1;
                    ae_v_moved(&work->ptr.p_float[n1], 1, &a->ptr.pp_float[jp1][n1], 1, ae_v_len(n1,n2), ctemp);
                    ae_v_subd(&work->ptr.p_float[n1], 1, &a->ptr.pp_float[j][n1], 1, ae_v_len(n1,n2), stemp);
                    ae_v_muld(&a->ptr.pp_float[j][n1], 1, ae_v_len(n1,n2), ctemp);
                    ae_v_addd(&a->ptr.pp_float[j][n1], 1, &a->ptr.pp_float[jp1][n1], 1, ae_v_len(n1,n2), stemp);
                    ae_v_move(&a->ptr.pp_float[jp1][n1], 1, &work->ptr.p_float[n1], 1, ae_v_len(n1,n2));
                }
            }
        }
        else
        {
            ...
            for(j=m1; j<=m2-1; j++)
            {
                ...
                if( ae_fp_neq(ctemp,(float)(1))||ae_fp_neq(stemp,(float)(0)) )
                {
                    temp = a->ptr.pp_float[j+1][n1];
                    a->ptr.pp_float[j+1][n1] = ctemp*temp-stemp*a->ptr.pp_float[j][n1];
                    a->ptr.pp_float[j][n1] = stemp*temp+ctemp*a->ptr.pp_float[j][n1];
                }
            }
        }
    }
     ...
     ae_state *_state)
{
    ...
    if( isforward )
    {
        if( m1!=m2 )
        {
            ...
            for(j=n1; j<=n2-1; j++)
            {
                ctemp = c->ptr.p_float[j-n1+1];
                stemp = s->ptr.p_float[j-n1+1];
                if( ae_fp_neq(ctemp,(float)(1))||ae_fp_neq(stemp,(float)(0)) )
                {
                    jp1 = j+1;
                    ae_v_moved(&work->ptr.p_float[m1], 1, &a->ptr.pp_float[m1][jp1], a->stride, ae_v_len(m1,m2), ctemp);
                    ae_v_subd(&work->ptr.p_float[m1], 1, &a->ptr.pp_float[m1][j], a->stride, ae_v_len(m1,m2), stemp);
                    ae_v_muld(&a->ptr.pp_float[m1][j], a->stride, ae_v_len(m1,m2), ctemp);
                    ae_v_addd(&a->ptr.pp_float[m1][j], a->stride, &a->ptr.pp_float[m1][jp1], a->stride, ae_v_len(m1,m2), stemp);
                    ae_v_move(&a->ptr.pp_float[m1][jp1], a->stride, &work->ptr.p_float[m1], 1, ae_v_len(m1,m2));
                }
            }
        }
        else
        {
            ...
            for(j=n1; j<=n2-1; j++)
            {
                ...
                if( ae_fp_neq(ctemp,(float)(1))||ae_fp_neq(stemp,(float)(0)) )
                {
                    temp = a->ptr.pp_float[m1][j+1];
                    a->ptr.pp_float[m1][j+1] = ctemp*temp-stemp*a->ptr.pp_float[m1][j];
                    a->ptr.pp_float[m1][j] = stemp*temp+ctemp*a->ptr.pp_float[m1][j];
                }
            }
        }
    }
    ...
    *r = 0;
    ...
    else
    {
        ...
        else
        {
            ...
            *cs = f1/(*r);
            *sn = g1/(*r);
            if( ae_fp_greater(ae_fabs(f, _state),ae_fabs(g, _state))&&ae_fp_less(*cs,(float)(0)) )
            {
                *cs = -*cs;
                *sn = -*sn;
                *r = -*r;
     ...
     ae_state *_state)
{
    ...
    *info = 0;
    ...
    for(i=0; i<=n-1; i++)
    {
        for(j=0; j<=n-1; j++)
        {
            h1.ptr.pp_float[1+i][1+j] = h->ptr.pp_float[i][j];
        }
    }
    if( zneeded==1 )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            for(j=0; j<=n-1; j++)
            {
                z1.ptr.pp_float[1+i][1+j] = z->ptr.pp_float[i][j];
            }
        }
    }
    ...
    for(i=0; i<=n-1; i++)
    {
        wr->ptr.p_float[i] = wr1.ptr.p_float[i+1];
        wi->ptr.p_float[i] = wi1.ptr.p_float[i+1];
    }
    if( tneeded!=0 )
    {
        for(i=0; i<=n-1; i++)
        {
            for(j=0; j<=n-1; j++)
            {
                h->ptr.pp_float[i][j] = h1.ptr.pp_float[1+i][1+j];
            }
        }
    }
    if( zneeded!=0 )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            for(j=0; j<=n-1; j++)
            {
                z->ptr.pp_float[i][j] = z1.ptr.pp_float[1+i][1+j];
            }
        }
    }
    ae_frame_leave(_state);
     ...
     ae_state *_state)
{
    ...
    *info = 0;
    ...
    if( initz )
    {
        ...
        for(i=1; i<=n; i++)
        {
            for(j=1; j<=n; j++)
            {
                if( i==j )
                {
                    z->ptr.pp_float[i][j] = (float)(1);
                }
                else
                {
                    z->ptr.pp_float[i][j] = (float)(0);
                }
            }
        }
    }
    ...
    if( n==1 )
    {
        wr->ptr.p_float[1] = h->ptr.pp_float[1][1];
        wi->ptr.p_float[1] = (float)(0);
        ae_frame_leave(_state);
        return;
    }
    ...
    if( (ns<=2||ns>n)||maxb>=n )
    {
        ...
        /*
         * Use the standard float-shift algorithm
         */
        ...
        if( wantt )
        {
            ...
            while(j<=n)
            {
                if( ae_fp_eq(wi->ptr.p_float[j],(float)(0)) )
                {
                    for(i=j+1; i<=n; i++)
                    {
                        h->ptr.pp_float[i][j] = (float)(0);
    ...
    for(;;)
    {
        ...
        for(its=0; its<=itn; its++)
        {
            ...
            for(k=i; k>=l+1; k--)
            {
                tst1 = ae_fabs(h->ptr.pp_float[k-1][k-1], _state)+ae_fabs(h->ptr.pp_float[k][k], _state);
                if( ae_fp_eq(tst1,(float)(0)) )
                {
                    tst1 = upperhessenberg1norm(h, l, i, l, i, &work, _state);
                }
                if( ae_fp_less_eq(ae_fabs(h->ptr.pp_float[k][k-1], _state),ae_maxreal(ulp*tst1, smlnum, _state)) )
                {
                    break;
                }
            }
            ...
            if( its==20||its==30 )
            {
                ...
                {
                    wr->ptr.p_float[ii] = cnst*(ae_fabs(h->ptr.pp_float[ii][ii-1], _state)+ae_fabs(h->ptr.pp_float[ii][ii], _state));
                    wi->ptr.p_float[ii] = (float)(0);
                }
            }
            else
            {
                ...
                for(p1=1; p1<=ns; p1++)
                {
                    wr->ptr.p_float[i-ns+p1] = tmpwr.ptr.p_float[p1];
                    wi->ptr.p_float[i-ns+p1] = tmpwi.ptr.p_float[p1];
                }
                if( ierr>0 )
                {
                    ...
                    for(ii=1; ii<=ierr; ii++)
                    {
                        wr->ptr.p_float[i-ns+ii] = s.ptr.pp_float[ii][ii];
                        wi->ptr.p_float[i-ns+ii] = (float)(0);
                    }
                }
            }
             ...
             */
            v.ptr.p_float[1] = (float)(1);
            for(ii=2; ii<=ns+1; ii++)
            {
                v.ptr.p_float[ii] = (float)(0);
            }
            nv = 1;
            for(j=i-ns+1; j<=i; j++)
            {
                if( ae_fp_greater_eq(wi->ptr.p_float[j],(float)(0)) )
                {
                    if( ae_fp_eq(wi->ptr.p_float[j],(float)(0)) )
                    {
                         ...
                         */
                        p1 = nv+1;
                        ae_v_move(&vv.ptr.p_float[1], 1, &v.ptr.p_float[1], 1, ae_v_len(1,p1));
                        matrixvectormultiply(h, l, l+nv, l, l+nv-1, ae_false, &vv, 1, nv, 1.0, &v, 1, nv+1, -wr->ptr.p_float[j], _state);
                        nv = nv+1;
                    }
                    else
                    {
                        if( ae_fp_greater(wi->ptr.p_float[j],(float)(0)) )
                        {
                            ...
                            p1 = nv+1;
                            ae_v_move(&vv.ptr.p_float[1], 1, &v.ptr.p_float[1], 1, ae_v_len(1,p1));
                            matrixvectormultiply(h, l, l+nv, l, l+nv-1, ae_false, &v, 1, nv, 1.0, &vv, 1, nv+1, -2*wr->ptr.p_float[j], _state);
                            itemp = vectoridxabsmax(&vv, 1, nv+1, _state);
                            temp = 1/ae_maxreal(ae_fabs(vv.ptr.p_float[itemp], _state), smlnum, _state);
                            p1 = nv+1;
                            ae_v_muld(&vv.ptr.p_float[1], 1, ae_v_len(1,p1), temp);
                            absw = pythag2(wr->ptr.p_float[j], wi->ptr.p_float[j], _state);
                            temp = temp*absw*absw;
                    ...
                    temp = ae_fabs(v.ptr.p_float[itemp], _state);
                    if( ae_fp_eq(temp,(float)(0)) )
                    {
                        v.ptr.p_float[1] = (float)(1);
                        for(ii=2; ii<=nv; ii++)
                        {
                    ...
                    else
                    {
                        ...
                        vt = 1/temp;
                        ae_v_muld(&v.ptr.p_float[1], 1, ae_v_len(1,nv), vt);
                    }
                }
            }
            ...
            for(k=l; k<=i-1; k++)
            {
                ...
                if( k>l )
                {
                    p1 = k-1;
                    p2 = k+nr-1;
                    ae_v_move(&v.ptr.p_float[1], 1, &h->ptr.pp_float[k][p1], h->stride, ae_v_len(1,nr));
                    touchint(&p2, _state);
                }
                generatereflection(&v, nr, &tau, _state);
                if( k>l )
                {
                    h->ptr.pp_float[k][k-1] = v.ptr.p_float[1];
                    for(ii=k+1; ii<=i; ii++)
                    {
                        h->ptr.pp_float[ii][k-1] = (float)(0);
                    }
                }
                v.ptr.p_float[1] = (float)(1);
         ...
         * off. Use the float-shift QR algorithm to handle it.
         */
     ...
     ae_state *_state)
{
    ...
    *info = 0;
    ...
    if( ilo==ihi )
    {
        wr->ptr.p_float[ilo] = h->ptr.pp_float[ilo][ilo];
        wi->ptr.p_float[ilo] = (float)(0);
        return;
    }
    nh = ihi-ilo+1;
    nz = ihiz-iloz+1;
    ...
    for(;;)
    {
        ...
        for(its=0; its<=itn; its++)
        {
            ...
            if( its==10||its==20 )
            {
                 ...
                 * Exceptional shift.
                 */
                s = ae_fabs(h->ptr.pp_float[i][i-1], _state)+ae_fabs(h->ptr.pp_float[i-1][i-2], _state);
                h44 = dat1*s+h->ptr.pp_float[i][i];
                h33 = h44;
                h43h34 = dat2*s*s;
            }
            else
            {
                 ...
                 */
                h44 = h->ptr.pp_float[i][i];
                h33 = h->ptr.pp_float[i-1][i-1];
                h43h34 = h->ptr.pp_float[i][i-1]*h->ptr.pp_float[i-1][i];
                s = h->ptr.pp_float[i-1][i-2]*h->ptr.pp_float[i-1][i-2];
                disc = (h33-h44)*0.5;
                disc = disc*disc+h43h34;
                if( ae_fp_greater(disc,(float)(0)) )
                {
                    ...
                    ave = 0.5*(h33+h44);
                    if( ae_fp_greater(ae_fabs(h33, _state)-ae_fabs(h44, _state),(float)(0)) )
                    {
                        h33 = h33*h44-h43h34;
            ...
            for(m=i-2; m>=l; m--)
            {
                 ...
                 * negligible.
                 */
                h11 = h->ptr.pp_float[m][m];
                h22 = h->ptr.pp_float[m+1][m+1];
                h21 = h->ptr.pp_float[m+1][m];
                h12 = h->ptr.pp_float[m][m+1];
                h44s = h44-h11;
                h33s = h33-h11;
                v1 = (h33s*h44s-h43h34)/h21+h12;
                v2 = h22-h11-h33s-h44s;
                v3 = h->ptr.pp_float[m+2][m+1];
                s = ae_fabs(v1, _state)+ae_fabs(v2, _state)+ae_fabs(v3, _state);
                v1 = v1/s;
                v2 = v2/s;
                v3 = v3/s;
                workv3->ptr.p_float[1] = v1;
                workv3->ptr.p_float[2] = v2;
                workv3->ptr.p_float[3] = v3;
                if( m==l )
                {
                    break;
                }
                h00 = h->ptr.pp_float[m-1][m-1];
                h10 = h->ptr.pp_float[m][m-1];
            ...
            for(k=m; k<=i-1; k++)
            {
                ...
                if( k>m )
                {
                    for(p1=1; p1<=nr; p1++)
                    {
                        workv3->ptr.p_float[p1] = h->ptr.pp_float[k+p1-1][k-1];
                    }
                }
                generatereflection(workv3, nr, &t1, _state);
                if( k>m )
                {
                    h->ptr.pp_float[k][k-1] = workv3->ptr.p_float[1];
                    h->ptr.pp_float[k+1][k-1] = (float)(0);
                    if( k<i-1 )
                    {
                        h->ptr.pp_float[k+2][k-1] = (float)(0);
                    }
                }
                else
                {
                    if( m>l )
                    {
                        h->ptr.pp_float[k][k-1] = -h->ptr.pp_float[k][k-1];
                    }
                }
                v2 = workv3->ptr.p_float[2];
                t2 = t1*v2;
                if( nr==3 )
                {
                    v3 = workv3->ptr.p_float[3];
                    t3 = t1*v3;
                    ...
                    for(j=k; j<=i2; j++)
                    {
                        sum = h->ptr.pp_float[k][j]+v2*h->ptr.pp_float[k+1][j]+v3*h->ptr.pp_float[k+2][j];
                        h->ptr.pp_float[k][j] = h->ptr.pp_float[k][j]-sum*t1;
                        h->ptr.pp_float[k+1][j] = h->ptr.pp_float[k+1][j]-sum*t2;
                        h->ptr.pp_float[k+2][j] = h->ptr.pp_float[k+2][j]-sum*t3;
                    }
                    ...
                    {
                        sum = h->ptr.pp_float[j][k]+v2*h->ptr.pp_float[j][k+1]+v3*h->ptr.pp_float[j][k+2];
                        h->ptr.pp_float[j][k] = h->ptr.pp_float[j][k]-sum*t1;
                        h->ptr.pp_float[j][k+1] = h->ptr.pp_float[j][k+1]-sum*t2;
                        h->ptr.pp_float[j][k+2] = h->ptr.pp_float[j][k+2]-sum*t3;
                    }
                    if( wantz )
                    {
                        ...
                        for(j=iloz; j<=ihiz; j++)
                        {
                            sum = z->ptr.pp_float[j][k]+v2*z->ptr.pp_float[j][k+1]+v3*z->ptr.pp_float[j][k+2];
                            z->ptr.pp_float[j][k] = z->ptr.pp_float[j][k]-sum*t1;
                            z->ptr.pp_float[j][k+1] = z->ptr.pp_float[j][k+1]-sum*t2;
                            z->ptr.pp_float[j][k+2] = z->ptr.pp_float[j][k+2]-sum*t3;
                        }
                    }
                }
                else
                {
                    if( nr==2 )
                    {
                        ...
                        for(j=k; j<=i2; j++)
                        {
                            sum = h->ptr.pp_float[k][j]+v2*h->ptr.pp_float[k+1][j];
                            h->ptr.pp_float[k][j] = h->ptr.pp_float[k][j]-sum*t1;
                            h->ptr.pp_float[k+1][j] = h->ptr.pp_float[k+1][j]-sum*t2;
                        }
                        ...
                        for(j=i1; j<=i; j++)
                        {
                            sum = h->ptr.pp_float[j][k]+v2*h->ptr.pp_float[j][k+1];
                            h->ptr.pp_float[j][k] = h->ptr.pp_float[j][k]-sum*t1;
                            h->ptr.pp_float[j][k+1] = h->ptr.pp_float[j][k+1]-sum*t2;
                        }
                        if( wantz )
                        {
                            ...
                            for(j=iloz; j<=ihiz; j++)
                            {
                                sum = z->ptr.pp_float[j][k]+v2*z->ptr.pp_float[j][k+1];
                                z->ptr.pp_float[j][k] = z->ptr.pp_float[j][k]-sum*t1;
                                z->ptr.pp_float[j][k+1] = z->ptr.pp_float[j][k+1]-sum*t2;
                            }
                        }
        ...
        if( l==i )
        {
             ...
             */
            wr->ptr.p_float[i] = h->ptr.pp_float[i][i];
            wi->ptr.p_float[i] = (float)(0);
        }
        else
        {
            if( l==i-1 )
            {
                 ...
                 */
                him1im1 = h->ptr.pp_float[i-1][i-1];
                him1i = h->ptr.pp_float[i-1][i];
                hiim1 = h->ptr.pp_float[i][i-1];
                hii = h->ptr.pp_float[i][i];
                hsschur_aux2x2schur(&him1im1, &him1i, &hiim1, &hii, &wrim1, &wiim1, &wri, &wii, &cs, &sn, _state);
                wr->ptr.p_float[i-1] = wrim1;
                wi->ptr.p_float[i-1] = wiim1;
                wr->ptr.p_float[i] = wri;
                wi->ptr.p_float[i] = wii;
                h->ptr.pp_float[i-1][i-1] = him1im1;
                h->ptr.pp_float[i-1][i] = him1i;
                h->ptr.pp_float[i][i-1] = hiim1;
                h->ptr.pp_float[i][i] = hii;
                if( wantt )
                {
                    ...
                    if( i2>i )
                    {
                        workc1->ptr.p_float[1] = cs;
                        works1->ptr.p_float[1] = sn;
     ...
     ae_state *_state)
{
    ...
    *sn = 0;
    ...
    else
    {
        ...
        {
            if( ae_fp_eq(*a-(*d),(float)(0))&&hsschur_extschursigntoone(*b, _state)!=hsschur_extschursigntoone(*c, _state) )
            {
                *cs = (float)(1);
                *sn = (float)(0);
            }
            else
            {
                ...
                else
                {
                    ...
                    *sn = -p/(tau*(*cs))*hsschur_extschursign((float)(1), sigma, _state);
                    ...
                    *a = temp;
                    *d = temp;
                    if( ae_fp_neq(*c,(float)(0)) )
                    {
                        if( ae_fp_neq(*b,(float)(0)) )
                        {
     ...
     ae_state *_state)
{
    ...
    *s = 0;
    ...
    for(i=1; i<=n; i++)
    {
        ae_v_move(&a1.ptr.pp_float[i][1], 1, &a->ptr.pp_float[i-1][0], 1, ae_v_len(1,n));
    }
    ae_v_move(&x1.ptr.p_float[1], 1, &x->ptr.p_float[0], 1, ae_v_len(1,n));
     ...
     */
    ae_v_move(&x->ptr.p_float[0], 1, &x1.ptr.p_float[1], 1, ae_v_len(0,n-1));
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
    ...
    *s = (float)(1);
    if( !normin )
    {
        ...
        if( upper )
        {
            ...
            for(j=1; j<=n; j++)
            {
                ...
                for(k=1; k<=j-1; k++)
                {
                    v = v+ae_fabs(a->ptr.pp_float[k][j], _state);
                }
                cnorm->ptr.p_float[j] = v;
            }
        }
        else
        {
    ...
    for(k=2; k<=n; k++)
    {
        if( ae_fp_greater(cnorm->ptr.p_float[k],cnorm->ptr.p_float[imax]) )
        {
            imax = k;
        }
    }
    tmax = cnorm->ptr.p_float[imax];
    ...
    {
        tscal = 1/(smlnum*tmax);
        ae_v_muld(&cnorm->ptr.p_float[1], 1, ae_v_len(1,n), tscal);
    }
    ...
    for(k=2; k<=n; k++)
    {
        if( ae_fp_greater(ae_fabs(x->ptr.p_float[k], _state),ae_fabs(x->ptr.p_float[j], _state)) )
        {
            j = k;
        }
    }
    xmax = ae_fabs(x->ptr.p_float[j], _state);
    xbnd = xmax;
    if( notran )
    {
        ...
        else
        {
            if( nounit )
            {
                ...
                while((jinc>0&&j<=jlast)||(jinc<0&&j>=jlast))
                {
                     ...
                     * M(j) = G(j-1) / abs(A(j,j))
                     */
                    tjj = ae_fabs(a->ptr.pp_float[j][j], _state);
                    xbnd = ae_minreal(xbnd, ae_minreal((float)(1), tjj, _state)*grow, _state);
                    if( ae_fp_greater_eq(tjj+cnorm->ptr.p_float[j],smlnum) )
                    {
                        
                        /*
                         * G(j) = G(j-1)*( 1 + CNORM(j) / abs(A(j,j)) )
                         */
                        grow = grow*(tjj/(tjj+cnorm->ptr.p_float[j]));
                    }
                    else
                    {
            ...
            else
            {
                 ...
                 */
                grow = ae_minreal((float)(1), 1/ae_maxreal(xbnd, smlnum, _state), _state);
                j = jfirst;
                while((jinc>0&&j<=jlast)||(jinc<0&&j>=jlast))
                {
                     ...
                     * G(j) = G(j-1)*( 1 + CNORM(j) )
                     */
                    grow = grow*(1/(1+cnorm->ptr.p_float[j]));
                    j = j+jinc;
                }
            }
        }
    ...
    if( ae_fp_greater(grow*tscal,smlnum) )
    {
        ...
        if( (upper&&notran)||(!upper&&!notran) )
        {
            if( nounit )
            {
                vd = a->ptr.pp_float[n][n];
            }
            else
            {
                vd = (float)(1);
            }
            x->ptr.p_float[n] = x->ptr.p_float[n]/vd;
            for(i=n-1; i>=1; i--)
            {
                ip1 = i+1;
                if( upper )
                {
                    v = ae_v_dotproduct(&a->ptr.pp_float[i][ip1], 1, &x->ptr.p_float[ip1], 1, ae_v_len(ip1,n));
                }
                else
                {
                    v = ae_v_dotproduct(&a->ptr.pp_float[ip1][i], a->stride, &x->ptr.p_float[ip1], 1, ae_v_len(ip1,n));
                }
                if( nounit )
                {
                    vd = a->ptr.pp_float[i][i];
                }
                else
                {
                    vd = (float)(1);
                }
                x->ptr.p_float[i] = (x->ptr.p_float[i]-v)/vd;
            }
        }
        else
        {
            if( nounit )
            {
                vd = a->ptr.pp_float[1][1];
            }
            else
            {
                vd = (float)(1);
            }
            x->ptr.p_float[1] = x->ptr.p_float[1]/vd;
            for(i=2; i<=n; i++)
            {
                im1 = i-1;
                if( upper )
                {
                    v = ae_v_dotproduct(&a->ptr.pp_float[1][i], a->stride, &x->ptr.p_float[1], 1, ae_v_len(1,im1));
                }
                else
                {
                    v = ae_v_dotproduct(&a->ptr.pp_float[i][1], 1, &x->ptr.p_float[1], 1, ae_v_len(1,im1));
                }
                if( nounit )
                {
                    vd = a->ptr.pp_float[i][i];
    ...
    else
    {
        ...
        if( ae_fp_greater(xmax,bignum) )
        {
             ...
             */
            *s = bignum/xmax;
            ae_v_muld(&x->ptr.p_float[1], 1, ae_v_len(1,n), *s);
            xmax = bignum;
        }
        if( notran )
        {
            ...
            while((jinc>0&&j<=jlast)||(jinc<0&&j>=jlast))
            {
                 ...
                 */
                xj = ae_fabs(x->ptr.p_float[j], _state);
                flg = 0;
                if( nounit )
                {
                    tjjs = a->ptr.pp_float[j][j]*tscal;
                }
                else
                {
                    tjjs = tscal;
                    if( ae_fp_eq(tscal,(float)(1)) )
                    {
                        flg = 100;
                    }
                }
                    ...
                    if( ae_fp_greater(tjj,smlnum) )
                    {
                        ...
                        if( ae_fp_less(tjj,(float)(1)) )
                        {
                            if( ae_fp_greater(xj,tjj*bignum) )
                            {
                                 ...
                                 */
                                rec = 1/xj;
                                ae_v_muld(&x->ptr.p_float[1], 1, ae_v_len(1,n), rec);
                                *s = *s*rec;
                                xmax = xmax*rec;
                            }
                        }
                        x->ptr.p_float[j] = x->ptr.p_float[j]/tjjs;
                        xj = ae_fabs(x->ptr.p_float[j], _state);
                    }
                    else
                    {
                        if( ae_fp_greater(tjj,(float)(0)) )
                        {
                            ...
                            if( ae_fp_greater(xj,tjj*bignum) )
                            {
                                 ...
                                 */
                                rec = tjj*bignum/xj;
                                if( ae_fp_greater(cnorm->ptr.p_float[j],(float)(1)) )
                                {
                        ...
                        else
                        {
                            ...
                            for(i=1; i<=n; i++)
                            {
                                x->ptr.p_float[i] = (float)(0);
                            }
                            x->ptr.p_float[j] = (float)(1);
                            xj = (float)(1);
                            *s = (float)(0);
                            xmax = (float)(0);
                        }
                 ...
                 */
                if( ae_fp_greater(xj,(float)(1)) )
                {
                    rec = 1/xj;
                    if( ae_fp_greater(cnorm->ptr.p_float[j],(bignum-xmax)*rec) )
                    {
                ...
                {
                    if( ae_fp_greater(xj*cnorm->ptr.p_float[j],bignum-xmax) )
                    {
                        
                        /*
                         * Scale x by 1/2.
                         */
                        ae_v_muld(&x->ptr.p_float[1], 1, ae_v_len(1,n), 0.5);
                        *s = *s*0.5;
                    }
                }
                if( upper )
                {
                    if( j>1 )
                    {
                         ...
                         */
                        v = x->ptr.p_float[j]*tscal;
                        jm1 = j-1;
                        ae_v_subd(&x->ptr.p_float[1], 1, &a->ptr.pp_float[1][j], a->stride, ae_v_len(1,jm1), v);
                        i = 1;
                        for(k=2; k<=j-1; k++)
                        {
                            if( ae_fp_greater(ae_fabs(x->ptr.p_float[k], _state),ae_fabs(x->ptr.p_float[i], _state)) )
                            {
                                i = k;
                            }
                        }
                        xmax = ae_fabs(x->ptr.p_float[i], _state);
                    }
                }
                else
                {
                    if( j<n )
                    {
                        ...
                        jp1 = j+1;
                        v = x->ptr.p_float[j]*tscal;
                        ae_v_subd(&x->ptr.p_float[jp1], 1, &a->ptr.pp_float[jp1][j], a->stride, ae_v_len(jp1,n), v);
                        i = j+1;
                        for(k=j+2; k<=n; k++)
                        {
                            if( ae_fp_greater(ae_fabs(x->ptr.p_float[k], _state),ae_fabs(x->ptr.p_float[i], _state)) )
        ...
        else
        {
            ...
            while((jinc>0&&j<=jlast)||(jinc<0&&j>=jlast))
            {
                ...
                xj = ae_fabs(x->ptr.p_float[j], _state);
                uscal = tscal;
                rec = 1/ae_maxreal(xmax, (float)(1), _state);
                if( ae_fp_greater(cnorm->ptr.p_float[j],(bignum-xj)*rec) )
                {
                    ...
                    tjj = ae_fabs(tjjs, _state);
                    if( ae_fp_greater(tjj,(float)(1)) )
                    {
                        
                        /*
                         * Divide by A(j,j) when scaling x if A(j,j) > 1.
                         */
                        rec = ae_minreal((float)(1), rec*tjj, _state);
                        uscal = uscal/tjjs;
                    }
                    if( ae_fp_less(rec,(float)(1)) )
                    {
                        ae_v_muld(&x->ptr.p_float[1], 1, ae_v_len(1,n), rec);
                        *s = *s*rec;
                        xmax = xmax*rec;
                ...
                if( ae_fp_eq(uscal,(float)(1)) )
                {
                    ...
                    if( upper )
                    {
                        ...
                        {
                            jm1 = j-1;
                            sumj = ae_v_dotproduct(&a->ptr.pp_float[1][j], a->stride, &x->ptr.p_float[1], 1, ae_v_len(1,jm1));
                        }
                        else
                        {
                            sumj = (float)(0);
                    ...
                    else
                    {
                        ...
                        {
                            jp1 = j+1;
                            sumj = ae_v_dotproduct(&a->ptr.pp_float[jp1][j], a->stride, &x->ptr.p_float[jp1], 1, ae_v_len(jp1,n));
                        }
                    }
                }
                else
                {
                    ...
                    if( upper )
                    {
                        for(i=1; i<=j-1; i++)
                        {
                            v = a->ptr.pp_float[i][j]*uscal;
                            sumj = sumj+v*x->ptr.p_float[i];
                        }
                    }
                    else
                    {
                ...
                if( ae_fp_eq(uscal,tscal) )
                {
                     ...
                     */
                    x->ptr.p_float[j] = x->ptr.p_float[j]-sumj;
                    xj = ae_fabs(x->ptr.p_float[j], _state);
                    flg = 0;
                    if( nounit )
                    {
                ...
                else
                {
                     ...
                     */
                    x->ptr.p_float[j] = x->ptr.p_float[j]/tjjs-sumj;
                }
                xmax = ae_maxreal(xmax, ae_fabs(x->ptr.p_float[j], _state), _state);
                j = j+jinc;
            }
        }
        *s = *s/tscal;
    ...
    {
        v = 1/tscal;
        ae_v_muld(&cnorm->ptr.p_float[1], 1, ae_v_len(1,n), v);
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        nrmb = ae_maxreal(nrmb, ae_fabs(x->ptr.p_float[i], _state), _state);
    }
    nrmx = (float)(0);
    ...
    if( isupper&&trans==0 )
    {
        ...
        for(i=n-1; i>=0; i--)
        {
            ...
            {
                alpha = ae_complex_from_d(a->ptr.pp_float[i][i]*sa);
            }
            if( i<n-1 )
            {
                ae_v_moved(&tmp.ptr.p_float[i+1], 1, &a->ptr.pp_float[i][i+1], 1, ae_v_len(i+1,n-1), sa);
                vr = ae_v_dotproduct(&tmp.ptr.p_float[i+1], 1, &x->ptr.p_float[i+1], 1, ae_v_len(i+1,n-1));
                beta = ae_complex_from_d(x->ptr.p_float[i]-vr);
            }
            else
            {
                beta = ae_complex_from_d(x->ptr.p_float[i]);
            }
    ...
    if( !isupper&&trans==0 )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            ...
            if( i>0 )
            {
                ae_v_moved(&tmp.ptr.p_float[0], 1, &a->ptr.pp_float[i][0], 1, ae_v_len(0,i-1), sa);
                vr = ae_v_dotproduct(&tmp.ptr.p_float[0], 1, &x->ptr.p_float[0], 1, ae_v_len(0,i-1));
                beta = ae_complex_from_d(x->ptr.p_float[i]-vr);
            }
            else
            {
    ...
    if( isupper&&trans==1 )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            ...
            if( i<n-1 )
            {
                vr = cx.x;
                ae_v_moved(&tmp.ptr.p_float[i+1], 1, &a->ptr.pp_float[i][i+1], 1, ae_v_len(i+1,n-1), sa);
                ae_v_subd(&x->ptr.p_float[i+1], 1, &tmp.ptr.p_float[i+1], 1, ae_v_len(i+1,n-1), vr);
            }
        }
        ae_frame_leave(_state);
        return result;
    ...
    if( !isupper&&trans==1 )
    {
        ...
        for(i=n-1; i>=0; i--)
        {
            ...
            if( i>0 )
            {
                vr = cx.x;
                ae_v_moved(&tmp.ptr.p_float[0], 1, &a->ptr.pp_float[i][0], 1, ae_v_len(0,i-1), sa);
                ae_v_subd(&x->ptr.p_float[0], 1, &tmp.ptr.p_float[0], 1, ae_v_len(0,i-1), vr);
            }
        }
        ae_frame_leave(_state);
        return result;
     ...
     ae_state *_state)
{
    ...
    if( !hpccores_hpcfinalizechunkedgradientx(&buf->hpcbuf, buf->wcount, grad, _state) )
    {
        ...
        {
            grad->ptr.p_float[i] = grad->ptr.p_float[i]+buf->hpcbuf.ptr.p_float[i];
        }
    }
}
...
OUTPUT PARAMETERS
    ...
    RErr    -   estimate of error. This estimate accounts for both  errors
                ...
                rounding of A and B to fit in float (about 1 ulp).
     ...
     ae_state *_state)
{
    ...
    *rerr = 0;
    ...
    for(i=0; i<=n-1; i++)
    {
        v = a->ptr.p_float[i]*b->ptr.p_float[i];
        temp->ptr.p_float[i] = v;
        mx = ae_maxreal(mx, ae_fabs(v, _state), _state);
    }
    if( ae_fp_eq(mx,(float)(0)) )
    {
        *r = (float)(0);
        *rerr = (float)(0);
        return;
     ...
     ae_state *_state)
{
    ...
    *rerr = 0;
    ...
    for(i=0; i<=n-1; i++)
    {
        ...
        temp->ptr.p_float[2*i+0] = v;
        mx = ae_maxreal(mx, ae_fabs(v, _state), _state);
        v = -a->ptr.p_complex[i].y*b->ptr.p_complex[i].y;
        temp->ptr.p_float[2*i+1] = v;
        mx = ae_maxreal(mx, ae_fabs(v, _state), _state);
    }
    if( ae_fp_eq(mx,(float)(0)) )
    {
     ...
     * total error
     */
    if( ae_fp_eq(rerrx,(float)(0))&&ae_fp_eq(rerry,(float)(0)) )
    {
        *rerr = (float)(0);
    }
    else
     ...
     ae_state *_state)
{
    ...
    *rerr = mx*ae_machineepsilon;
    ...
    s = xblas_xfastpow((float)(2), -k, _state);
    while(ae_fp_greater_eq(s*mx,(float)(1)))
    {
        s = 0.5*s;
    }
    ...
    ae_v_muld(&w->ptr.p_float[0], 1, ae_v_len(0,n-1), s);
    s = 1/s;
     ...
     */
    k = ae_trunc(ae_log((float)536870912/(float)n, _state)/ln2, _state);
    chunk = xblas_xfastpow((float)(2), k, _state);
    if( ae_fp_less(chunk,(float)(2)) )
    {
        chunk = (float)(2);
    }
    invchunk = 1/chunk;
     ...
     */
    *r = (float)(0);
    ae_v_muld(&w->ptr.p_float[0], 1, ae_v_len(0,n-1), chunk);
    for(;;)
    {
        s = s*invchunk;
        allzeros = ae_true;
        ...
        for(i=0; i<=n-1; i++)
        {
            v = w->ptr.p_float[i];
            k = ae_trunc(v, _state);
            if( ae_fp_neq(v,(float)(k)) )
            {
                allzeros = ae_false;
            }
            w->ptr.p_float[i] = chunk*(v-k);
            ks = ks+k;
        }
        *r = *r+s*ks;
        v = ae_fabs(*r, _state);
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        mx = ae_maxreal(mx, ae_fabs(d->ptr.p_float[i], _state), _state);
    }
    if( ae_fp_eq(mx,(float)(0)) )
    {
        return;
    ...
    s = 1/mx;
    ae_v_muld(&d->ptr.p_float[0], 1, ae_v_len(0,n-1), s);
    *stp = *stp/s;
    
    /*
     * normalize D
     */
    s = ae_v_dotproduct(&d->ptr.p_float[0], 1, &d->ptr.p_float[0], 1, ae_v_len(0,n-1));
    ...
    for(;;)
    {
        ...
        if( *stage==2 )
        {
            ...
            *info = 0;
             ...
             */
            if( ae_fp_less(stpmax,linmin_stpmin)&&ae_fp_greater(stpmax,(float)(0)) )
            {
                *info = 5;
                *stp = stpmax;
                *stage = 0;
            ...
            if( ((((((n<=0||ae_fp_less_eq(*stp,(float)(0)))||ae_fp_less(linmin_ftol,(float)(0)))||ae_fp_less(gtol,zero))||ae_fp_less(linmin_xtol,zero))||ae_fp_less(linmin_stpmin,zero))||ae_fp_less(stpmax,linmin_stpmin))||linmin_maxfev<=0 )
            {
                *stage = 0;
                return;
            }
             ...
             */
            v = ae_v_dotproduct(&g->ptr.p_float[0], 1, &s->ptr.p_float[0], 1, ae_v_len(0,n-1));
            state->dginit = v;
            if( ae_fp_greater_eq(state->dginit,(float)(0)) )
            {
                *stage = 0;
                return;
            }
            ...
            *nfev = 0;
            ...
            ae_v_move(&wa->ptr.p_float[0], 1, &x->ptr.p_float[0], 1, ae_v_len(0,n-1));
        ...
        if( *stage==3 )
        {
             ...
             */
            ae_v_move(&x->ptr.p_float[0], 1, &wa->ptr.p_float[0], 1, ae_v_len(0,n-1));
            ae_v_addd(&x->ptr.p_float[0], 1, &s->ptr.p_float[0], 1, ae_v_len(0,n-1), *stp);
            ...
            *nfev = *nfev+1;
            ...
            if( *info!=0 )
            {
                ...
                if( *info==1||*info==5 )
                {
                    ...
                    for(i=0; i<=n-1; i++)
                    {
                        v = v+(wa->ptr.p_float[i]-x->ptr.p_float[i])*(wa->ptr.p_float[i]-x->ptr.p_float[i]);
                    }
     ...
     ae_state *_state)
{
    ...
    state->fcur = f;
    state->n = n;
    ae_v_move(&state->xbase.ptr.p_float[0], 1, &x->ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_move(&state->s.ptr.p_float[0], 1, &s->ptr.p_float[0], 1, ae_v_len(0,n-1));
...
ae_bool armijoiteration(armijostate* state, ae_state *_state)
{
    ...
    if( state->rstate.stage>=0 )
    {
        ...
        v = state->rstate.ra.ptr.p_float[0];
    }
    else
    {
        n = -983;
     ...
     * Routine body
     */
    if( (ae_fp_less_eq(state->stplen,(float)(0))||ae_fp_less(state->stpmax,(float)(0)))||state->fmax<2 )
    {
        state->info = 0;
        result = ae_false;
        return result;
     ...
     * Bound StpLen
     */
    if( ae_fp_greater(state->stplen,state->stpmax)&&ae_fp_neq(state->stpmax,(float)(0)) )
    {
    ...
    if( ae_fp_greater(v,state->stpmax)&&ae_fp_neq(state->stpmax,(float)(0)) )
    {
        v = state->stpmax;
    }
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->xbase.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_addd(&state->x.ptr.p_float[0], 1, &state->s.ptr.p_float[0], 1, ae_v_len(0,n-1), v);
...
lbl_rcomm:
    ...
    state->rstate.ra.ptr.p_float[0] = v;
    return result;
}
     ...
     ae_state *_state)
{
    ...
    *info = 0;
     ...
     *     CHECK THE INPUT PARAMETERS FOR ERRORS.
     */
    if( ((*brackt&&(ae_fp_less_eq(*stp,ae_minreal(*stx, *sty, _state))||ae_fp_greater_eq(*stp,ae_maxreal(*stx, *sty, _state))))||ae_fp_greater_eq(*dx*(*stp-(*stx)),(float)(0)))||ae_fp_less(stmax,stmin) )
    {
        return;
    }
        ...
        else
        {
            if( ae_fp_less(ae_fabs(dp, _state),ae_fabs(*dx, _state)) )
            {
                ...
                *info = 3;
                 ...
                 */
                gamma = s*ae_sqrt(ae_maxreal((float)(0), ae_sqr(theta/s, _state)-*dx/s*(dp/s), _state), _state);
                if( ae_fp_greater(*stp,*stx) )
                {
                    gamma = -gamma;
                }
                ...
                q = gamma+(*dx-dp)+gamma;
                r = p/q;
                if( ae_fp_less(r,(float)(0))&&ae_fp_neq(gamma,(float)(0)) )
                {
                    stpc = *stp+r*(*stx-(*stp));
                }
                else
...
N should be power of 2, estimates are badly wrong for non-power-of-2 N's.
  ...
  -- ALGLIB --
     Copyright 01.05.2009 by Bochkanov Sergey
*************************************************************************/
float ftbasegetflopestimate(ae_int_t n, ae_state *_state)
{
    float result;


    result = ftbase_ftbaseinefficiencyfactor*(4*n*ae_log((float)(n), _state)/ae_log((float)(2), _state)-6*n+8);
    return result;
}
     ...
     ae_state *_state)
{
    ...
    for(opidx=0; opidx<=operandscnt-1; opidx++)
    {
        for(i=0; i<=n-1; i++)
        {
            ...
            for(k=0; k<=n-1; k++)
            {
                re = a->ptr.p_float[offs+opidx*operandsize*2+2*k+0];
                im = a->ptr.p_float[offs+opidx*operandsize*2+2*k+1];
                c = ae_cos(-2*ae_pi*k*i/n, _state);
                s = ae_sin(-2*ae_pi*k*i/n, _state);
                hre = hre+c*re-s*im;
                him = him+c*im+s*re;
            }
            buf->ptr.p_float[2*i+0] = hre;
            buf->ptr.p_float[2*i+1] = him;
        }
        for(i=0; i<=operandsize*2-1; i++)
        {
            a->ptr.p_float[offs+opidx*operandsize*2+i] = buf->ptr.p_float[i];
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    if( n==2 )
    {
        for(opidx=0; opidx<=operandscnt-1; opidx++)
        {
            ...
            a0x = a->ptr.p_float[aoffset+0];
            a0y = a->ptr.p_float[aoffset+1];
            a1x = a->ptr.p_float[aoffset+2];
            a1y = a->ptr.p_float[aoffset+3];
            v0 = a0x+a1x;
            v1 = a0y+a1y;
            v2 = a0x-a1x;
            v3 = a0y-a1y;
            a->ptr.p_float[aoffset+0] = v0;
            a->ptr.p_float[aoffset+1] = v1;
            a->ptr.p_float[aoffset+2] = v2;
            a->ptr.p_float[aoffset+3] = v3;
        }
        return;
    }
    if( n==3 )
    {
        ...
        for(opidx=0; opidx<=operandscnt-1; opidx++)
        {
            ...
            a1x = a->ptr.p_float[aoffset+2];
            a1y = a->ptr.p_float[aoffset+3];
            a2x = a->ptr.p_float[aoffset+4];
            a2y = a->ptr.p_float[aoffset+5];
            t1x = a1x+a2x;
            t1y = a1y+a2y;
            a0x = a0x+t1x;
            a0y = a0y+t1y;
            ...
            a2x = s1x-m2x;
            a2y = s1y-m2y;
            a->ptr.p_float[aoffset+0] = a0x;
            a->ptr.p_float[aoffset+1] = a0y;
            a->ptr.p_float[aoffset+2] = a1x;
            a->ptr.p_float[aoffset+3] = a1y;
            a->ptr.p_float[aoffset+4] = a2x;
            a->ptr.p_float[aoffset+5] = a2y;
        }
        return;
    }
    if( n==4 )
    {
        for(opidx=0; opidx<=operandscnt-1; opidx++)
        {
            ...
            a2x = a->ptr.p_float[aoffset+4];
            a2y = a->ptr.p_float[aoffset+5];
            a3x = a->ptr.p_float[aoffset+6];
            a3y = a->ptr.p_float[aoffset+7];
            t1x = a0x+a2x;
            t1y = a0y+a2y;
            t2x = a1x+a3x;
            t2y = a1y+a3y;
            ...
            m3x = a1y-a3y;
            m3y = a3x-a1x;
            a->ptr.p_float[aoffset+0] = t1x+t2x;
            a->ptr.p_float[aoffset+1] = t1y+t2y;
            a->ptr.p_float[aoffset+4] = t1x-t2x;
            a->ptr.p_float[aoffset+5] = t1y-t2y;
            a->ptr.p_float[aoffset+2] = m2x+m3x;
            a->ptr.p_float[aoffset+3] = m2y+m3y;
            a->ptr.p_float[aoffset+6] = m2x-m3x;
            a->ptr.p_float[aoffset+7] = m2y-m3y;
        }
        return;
    }
    if( n==5 )
    {
        ...
        {
            aoffset = offs+opidx*operandsize*2;
            t1x = a->ptr.p_float[aoffset+2]+a->ptr.p_float[aoffset+8];
            t1y = a->ptr.p_float[aoffset+3]+a->ptr.p_float[aoffset+9];
            t2x = a->ptr.p_float[aoffset+4]+a->ptr.p_float[aoffset+6];
            t2y = a->ptr.p_float[aoffset+5]+a->ptr.p_float[aoffset+7];
            t3x = a->ptr.p_float[aoffset+2]-a->ptr.p_float[aoffset+8];
            t3y = a->ptr.p_float[aoffset+3]-a->ptr.p_float[aoffset+9];
            t4x = a->ptr.p_float[aoffset+6]-a->ptr.p_float[aoffset+4];
            t4y = a->ptr.p_float[aoffset+7]-a->ptr.p_float[aoffset+5];
            t5x = t1x+t2x;
            t5y = t1y+t2y;
            a->ptr.p_float[aoffset+0] = a->ptr.p_float[aoffset+0]+t5x;
            a->ptr.p_float[aoffset+1] = a->ptr.p_float[aoffset+1]+t5y;
            m1x = c1*t5x;
            m1y = c1*t5y;
            m2x = c2*(t1x-t2x);
            m2y = c2*(t1y-t2y);
            ...
            s5x = m3x+m5x;
            s5y = m3y+m5y;
            s1x = a->ptr.p_float[aoffset+0]+m1x;
            s1y = a->ptr.p_float[aoffset+1]+m1y;
            s2x = s1x+m2x;
            s2y = s1y+m2y;
            s4x = s1x-m2x;
            s4y = s1y-m2y;
            a->ptr.p_float[aoffset+2] = s2x+s3x;
            a->ptr.p_float[aoffset+3] = s2y+s3y;
            a->ptr.p_float[aoffset+4] = s4x+s5x;
            a->ptr.p_float[aoffset+5] = s4y+s5y;
            a->ptr.p_float[aoffset+6] = s4x-s5x;
            a->ptr.p_float[aoffset+7] = s4y-s5y;
            a->ptr.p_float[aoffset+8] = s2x-s3x;
            a->ptr.p_float[aoffset+9] = s2y-s3y;
        }
        return;
    }
    if( n==6 )
    {
        ...
        for(opidx=0; opidx<=operandscnt-1; opidx++)
        {
            ...
            a3x = a->ptr.p_float[aoffset+6];
            a3y = a->ptr.p_float[aoffset+7];
            a4x = a->ptr.p_float[aoffset+8];
            a4y = a->ptr.p_float[aoffset+9];
            a5x = a->ptr.p_float[aoffset+10];
            a5y = a->ptr.p_float[aoffset+11];
            v0 = a0x;
            v1 = a0y;
            a0x = a0x+a3x;
            a0y = a0y+a3y;
            ...
            a->ptr.p_float[aoffset+0] = a0x;
            a->ptr.p_float[aoffset+1] = a0y;
            a->ptr.p_float[aoffset+2] = a3x;
            a->ptr.p_float[aoffset+3] = a3y;
            a->ptr.p_float[aoffset+4] = a1x;
            a->ptr.p_float[aoffset+5] = a1y;
            a->ptr.p_float[aoffset+6] = a4x;
            a->ptr.p_float[aoffset+7] = a4y;
            a->ptr.p_float[aoffset+8] = a2x;
            a->ptr.p_float[aoffset+9] = a2y;
            a->ptr.p_float[aoffset+10] = a5x;
            a->ptr.p_float[aoffset+11] = a5y;
        }
        return;
    }
}
     ...
     ae_state *_state)
{
    ...
    if( n==2 )
    {
        ...
        for(opidx=0; opidx<=operandscnt-1; opidx++)
        {
            ...
            {
                a0x = a->ptr.p_float[aoffset0];
                a0y = a->ptr.p_float[aoffset0+1];
                a1x = a->ptr.p_float[aoffset2];
                a1y = a->ptr.p_float[aoffset2+1];
                v0 = a0x+a1x;
                v1 = a0y+a1y;
                v2 = a0x-a1x;
                v3 = a0y-a1y;
                a->ptr.p_float[aoffset0] = v0;
                a->ptr.p_float[aoffset0+1] = v1;
                a->ptr.p_float[aoffset2] = v2*(1+twxm1)-v3*twy;
                a->ptr.p_float[aoffset2+1] = v3*(1+twxm1)+v2*twy;
                aoffset0 = aoffset0+2;
                aoffset2 = aoffset2+2;
    ...
    if( n==3 )
    {
        ...
        for(opidx=0; opidx<=operandscnt-1; opidx++)
        {
            ...
            for(mvidx=0; mvidx<=m-1; mvidx++)
            {
                ...
                a1x = a->ptr.p_float[aoffset2];
                a1y = a->ptr.p_float[aoffset2+1];
                a2x = a->ptr.p_float[aoffset4];
                a2y = a->ptr.p_float[aoffset4+1];
                t1x = a1x+a2x;
                t1y = a1y+a2y;
                a0x = a0x+t1x;
                a0y = a0y+t1y;
                ...
                tw2y = 2*twx*twy;
                a->ptr.p_float[aoffset0] = a0x;
                a->ptr.p_float[aoffset0+1] = a0y;
                a->ptr.p_float[aoffset2] = a1x*twx-a1y*twy;
                a->ptr.p_float[aoffset2+1] = a1y*twx+a1x*twy;
                a->ptr.p_float[aoffset4] = a2x*tw2x-a2y*tw2y;
                a->ptr.p_float[aoffset4+1] = a2y*tw2x+a2x*tw2y;
                aoffset0 = aoffset0+2;
                aoffset2 = aoffset2+2;
                aoffset4 = aoffset4+2;
                if( (mvidx+1)%ftbase_updatetw==0 )
    ...
    if( n==4 )
    {
        ...
        for(opidx=0; opidx<=operandscnt-1; opidx++)
        {
            ...
            for(mvidx=0; mvidx<=m-1; mvidx++)
            {
                ...
                a2x = a->ptr.p_float[aoffset4];
                a2y = a->ptr.p_float[aoffset4+1];
                a3x = a->ptr.p_float[aoffset6];
                a3y = a->ptr.p_float[aoffset6+1];
                t1x = a0x+a2x;
                t1y = a0y+a2y;
                t2x = a1x+a3x;
                t2y = a1y+a3y;
                ...
                a3x = m2x-m3x;
                a3y = m2y-m3y;
                a->ptr.p_float[aoffset0] = t1x+t2x;
                a->ptr.p_float[aoffset0+1] = t1y+t2y;
                a->ptr.p_float[aoffset2] = a1x*twx-a1y*twy;
                a->ptr.p_float[aoffset2+1] = a1y*twx+a1x*twy;
                a->ptr.p_float[aoffset4] = a2x*tw2x-a2y*tw2y;
                a->ptr.p_float[aoffset4+1] = a2y*tw2x+a2x*tw2y;
                a->ptr.p_float[aoffset6] = a3x*tw3x-a3y*tw3y;
                a->ptr.p_float[aoffset6+1] = a3y*tw3x+a3x*tw3y;
                aoffset0 = aoffset0+2;
                aoffset2 = aoffset2+2;
                aoffset4 = aoffset4+2;
                aoffset6 = aoffset6+2;
    ...
    if( n==5 )
    {
        ...
        for(opidx=0; opidx<=operandscnt-1; opidx++)
        {
            ...
            for(mvidx=0; mvidx<=m-1; mvidx++)
            {
                ...
                a3x = a->ptr.p_float[aoffset6];
                a3y = a->ptr.p_float[aoffset6+1];
                a4x = a->ptr.p_float[aoffset8];
                a4y = a->ptr.p_float[aoffset8+1];
                t1x = a1x+a4x;
                t1y = a1y+a4y;
                t2x = a2x+a3x;
                t2y = a2y+a3y;
                ...
                a4x = s2x-s3x;
                a4y = s2y-s3y;
                a->ptr.p_float[aoffset0] = q0x;
                a->ptr.p_float[aoffset0+1] = q0y;
                a->ptr.p_float[aoffset2] = a1x*twx-a1y*twy;
                a->ptr.p_float[aoffset2+1] = a1x*twy+a1y*twx;
                a->ptr.p_float[aoffset4] = a2x*tw2x-a2y*tw2y;
                a->ptr.p_float[aoffset4+1] = a2x*tw2y+a2y*tw2x;
                a->ptr.p_float[aoffset6] = a3x*tw3x-a3y*tw3y;
                a->ptr.p_float[aoffset6+1] = a3x*tw3y+a3y*tw3x;
                a->ptr.p_float[aoffset8] = a4x*tw4x-a4y*tw4y;
                a->ptr.p_float[aoffset8+1] = a4x*tw4y+a4y*tw4x;
                aoffset0 = aoffset0+2;
                aoffset2 = aoffset2+2;
                aoffset4 = aoffset4+2;
                aoffset6 = aoffset6+2;
    ...
    if( n==6 )
    {
        ...
        for(opidx=0; opidx<=operandscnt-1; opidx++)
        {
            ...
            for(mvidx=0; mvidx<=m-1; mvidx++)
            {
                a0x = a->ptr.p_float[aoffset0+0];
                a0y = a->ptr.p_float[aoffset0+1];
                a1x = a->ptr.p_float[aoffset2+0];
                a1y = a->ptr.p_float[aoffset2+1];
                a2x = a->ptr.p_float[aoffset4+0];
                a2y = a->ptr.p_float[aoffset4+1];
                a3x = a->ptr.p_float[aoffset6+0];
                a3y = a->ptr.p_float[aoffset6+1];
                a4x = a->ptr.p_float[aoffset8+0];
                a4y = a->ptr.p_float[aoffset8+1];
                a5x = a->ptr.p_float[aoffset10+0];
                a5y = a->ptr.p_float[aoffset10+1];
                v0 = a0x;
                v1 = a0y;
                a0x = a0x+a3x;
                a0y = a0y+a3y;
                ...
                a->ptr.p_float[aoffset0+0] = a0x;
                a->ptr.p_float[aoffset0+1] = a0y;
                a->ptr.p_float[aoffset2+0] = a3x*twx-a3y*twy;
                a->ptr.p_float[aoffset2+1] = a3y*twx+a3x*twy;
                a->ptr.p_float[aoffset4+0] = a1x*tw2x-a1y*tw2y;
                a->ptr.p_float[aoffset4+1] = a1y*tw2x+a1x*tw2y;
                a->ptr.p_float[aoffset6+0] = a4x*tw3x-a4y*tw3y;
                a->ptr.p_float[aoffset6+1] = a4y*tw3x+a4x*tw3y;
                a->ptr.p_float[aoffset8+0] = a2x*tw4x-a2y*tw4y;
                a->ptr.p_float[aoffset8+1] = a2y*tw4x+a2x*tw4y;
                a->ptr.p_float[aoffset10+0] = a5x*tw5x-a5y*tw5y;
                a->ptr.p_float[aoffset10+1] = a5y*tw5x+a5x*tw5y;
                aoffset0 = aoffset0+2;
                aoffset2 = aoffset2+2;
                aoffset4 = aoffset4+2;
                aoffset6 = aoffset6+2;
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=2*m-1; i++)
    {
        precr->ptr.p_float[offs+i] = (float)(0);
    }
    for(i=0; i<=n-1; i++)
    {
        bx = ae_cos(ae_pi/n*i*i, _state);
        by = ae_sin(ae_pi/n*i*i, _state);
        precr->ptr.p_float[offs+2*i+0] = bx;
        precr->ptr.p_float[offs+2*i+1] = by;
        precr->ptr.p_float[offs+2*((m-i)%m)+0] = bx;
        precr->ptr.p_float[offs+2*((m-i)%m)+1] = by;
    }
    ...
    for(i=0; i<=2*m-1; i++)
    {
        precr->ptr.p_float[offs+2*m+i] = precr->ptr.p_float[offs+i];
    }
     ...
     ae_state *_state)
{
    ...
    for(op=0; op<=operandscnt-1; op++)
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            x = a->ptr.p_float[p0+0];
            y = a->ptr.p_float[p0+1];
            bx = plan->precr.ptr.p_float[p1+0];
            by = -plan->precr.ptr.p_float[p1+1];
            bufa->ptr.p_float[2*i+0] = x*bx-y*by;
            bufa->ptr.p_float[2*i+1] = x*by+y*bx;
            p0 = p0+2;
            p1 = p1+2;
        }
        for(i=2*n; i<=2*m-1; i++)
        {
            bufa->ptr.p_float[i] = (float)(0);
        }
        ...
        for(i=0; i<=m-1; i++)
        {
            ax = bufa->ptr.p_float[p0+0];
            ay = bufa->ptr.p_float[p0+1];
            bx = plan->precr.ptr.p_float[p1+0];
            by = plan->precr.ptr.p_float[p1+1];
            bufa->ptr.p_float[p0+0] = ax*bx-ay*by;
            bufa->ptr.p_float[p0+1] = -(ax*by+ay*bx);
            p0 = p0+2;
            p1 = p1+2;
        }
        ftbase_ftapplysubplan(plan, subplan, bufa, 0, 0, bufc, 1, _state);
        ...
        for(i=0; i<=n-1; i++)
        {
            bx = plan->precr.ptr.p_float[p0+0];
            by = plan->precr.ptr.p_float[p0+1];
            rx = bufa->ptr.p_float[p1+0]/m;
            ry = -bufa->ptr.p_float[p1+1]/m;
            a->ptr.p_float[p2+0] = rx*bx-ry*(-by);
            a->ptr.p_float[p2+1] = rx*(-by)+ry*bx;
            p0 = p0+2;
            p1 = p1+2;
            p2 = p2+2;
        }
     ...
     ae_state *_state)
{
    ...
    {
        v = -2*ae_pi*kiq/n;
        precr->ptr.p_float[offs+2*q+0] = ae_cos(v, _state);
        precr->ptr.p_float[offs+2*q+1] = ae_sin(v, _state);
        kiq = kiq*riq%n;
    }
     ...
     ae_state *_state)
{
    ...
    for(opidx=0; opidx<=operandscnt-1; opidx++)
    {
        ...
        rx = a->ptr.p_float[p0+0];
        ry = a->ptr.p_float[p0+1];
        x0 = rx;
        y0 = ry;
        for(q=0; q<=n-2; q++)
        {
            ax = a->ptr.p_float[p0+2*kq+0];
            ay = a->ptr.p_float[p0+2*kq+1];
            buf->ptr.p_float[p1+0] = ax;
            buf->ptr.p_float[p1+1] = ay;
            rx = rx+ax;
            ry = ry+ay;
            kq = kq*rq%n;
            p1 = p1+2;
        ...
        for(q=0; q<=n-2; q++)
        {
            a->ptr.p_float[p0] = buf->ptr.p_float[p1];
            a->ptr.p_float[p0+1] = buf->ptr.p_float[p1+1];
            p0 = p0+2;
            p1 = p1+2;
        }
        ...
        for(i=0; i<=n-2; i++)
        {
            ax = a->ptr.p_float[p0+0];
            ay = a->ptr.p_float[p0+1];
            bx = plan->precr.ptr.p_float[p1+0];
            by = plan->precr.ptr.p_float[p1+1];
            a->ptr.p_float[p0+0] = ax*bx-ay*by;
            a->ptr.p_float[p0+1] = -(ax*by+ay*bx);
            p0 = p0+2;
            p1 = p1+2;
        }
        ftbase_ftapplysubplan(plan, subplan, a, abase, aoffset+opidx*n*2, buf, 1, _state);
        ...
        for(i=0; i<=n-2; i++)
        {
            a->ptr.p_float[p0+0] = a->ptr.p_float[p0+0]/(n-1);
            a->ptr.p_float[p0+1] = -a->ptr.p_float[p0+1]/(n-1);
            p0 = p0+2;
        }
         ...
         * Result
         */
        buf->ptr.p_float[aoffset+opidx*n*2+0] = rx;
        buf->ptr.p_float[aoffset+opidx*n*2+1] = ry;
        kiq = 1;
        p0 = aoffset+opidx*n*2;
        p1 = abase+aoffset+opidx*n*2;
        for(q=0; q<=n-2; q++)
        {
            buf->ptr.p_float[p0+2*kiq+0] = x0+a->ptr.p_float[p1+0];
            buf->ptr.p_float[p0+2*kiq+1] = y0+a->ptr.p_float[p1+1];
            kiq = kiq*riq%n;
            p1 = p1+2;
        }
        p0 = abase+aoffset+opidx*n*2;
     ...
     ae_state *_state)
{
    ...
    *n2 = 0;
    ...
    if( n>ftbase_recursivethreshold )
    {
        k = ae_iceil(ae_sqrt((float)(n), _state), _state)+1;
...
static ae_int_t ftbase_ftoptimisticestimate(ae_int_t n, ae_state *_state)
{
    ...
    result = ae_ifloor(1.0E-5*5*n*ae_log((float)(n), _state)/ae_log((float)(2), _state), _state);
    return result;
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n2-1; i++)
    {
        ...
        for(j2=0; j2<=halfn1-1; j2++)
        {
             ...
             */
            x = a->ptr.p_float[offs+0];
            y = a->ptr.p_float[offs+1];
            tmpx = x*(1+twxm1)-y*twy;
            tmpy = x*twy+y*(1+twxm1);
            a->ptr.p_float[offs+0] = tmpx;
            a->ptr.p_float[offs+1] = tmpy;
            tmpx = (1+twxm1)*twrowxm1-twy*twrowy;
            twy = twy+(1+twxm1)*twrowy+twy*twrowxm1;
            twxm1 = twxm1+tmpx;
            x = a->ptr.p_float[offs+2];
            y = a->ptr.p_float[offs+3];
            tmpx = x*(1+twxm1)-y*twy;
            tmpy = x*twy+y*(1+twxm1);
            a->ptr.p_float[offs+2] = tmpx;
            a->ptr.p_float[offs+3] = tmpy;
            offs = offs+4;
     ...
     ae_state *_state)
{
    ...
    ae_v_move(&a->ptr.p_float[astart], 1, &buf->ptr.p_float[0], 1, ae_v_len(astart,astart+2*m*n-1));
}
     ...
     ae_state *_state)
{
    ...
    if( ae_maxint(m, n, _state)<=8 )
    {
        ...
        for(i=0; i<=m-1; i++)
        {
            ...
            for(j=0; j<=n-1; j++)
            {
                b->ptr.p_float[idx1+0] = a->ptr.p_float[idx2+0];
                b->ptr.p_float[idx1+1] = a->ptr.p_float[idx2+1];
                idx1 = idx1+m2;
                idx2 = idx2+2;
            }
        }
     ...
     ae_state *_state)
{
    ...
    if( ae_maxint(m, n, _state)<=8 )
    {
        for(i=0; i<=m-1; i++)
        {
            ...
            for(j=0; j<=n-1; j++)
            {
                b->ptr.p_float[idx1] = a->ptr.p_float[idx2];
                idx1 = idx1+bstride;
                idx2 = idx2+1;
            }
        }
...
float nulog1p(float x, ae_state *_state)
{
    float z;
    float lp;
    float lq;
...
float nuexpm1(float x, ae_state *_state)
{
    float r;
    float xx;
    float ep;
...
float nucosm1(float x, ae_state *_state)
{
    float xx;
    float c;
    float result;
